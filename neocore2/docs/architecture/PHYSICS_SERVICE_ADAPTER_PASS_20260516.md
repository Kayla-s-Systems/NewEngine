...
bad:
  step(&mut World)
  store PhysicsWorldService in ECS resources
...
Physics backend selection is driven by manifest/capability/service metadata:
...
names. If a provider must be visible before ABI descriptor probing, that metadata
comes from the runtime plugin manifest generated by the plugin build pipeline.

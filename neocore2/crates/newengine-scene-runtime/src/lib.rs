...
fn reject_ytyp_scene_path(path: &str) -> Result<(), String> {
    ...
    if lower.ends_with(".ytyp") {
        return Err(format!(
            "engine.scene load_json_v1 cannot load '{path}' as a scene path: .ytyp is owned by engine.definitions, not engine.scene"
        ));
    }
    Ok(())
}
...
fn validate_definition_ref_through_gateways(definition_ref: &str) -> Result<(), String> {
    ...
    let graph = call_gateway_json(
        newengine_assets_api::ENGINE_ASSET_GRAPH_SERVICE_ID,
    ...
    if graph.get("root_ref").and_then(|v| v.as_str()).unwrap_or("").is_empty() {
        return Err(format!("engine.asset_graph returned no root_ref for scene definition_ref='{}'", reference.canonical));
    }

    let validation = call_gateway_json(
        newengine_assets_api::ENGINE_DEFINITIONS_SERVICE_ID,
    ...
    if !validation.get("ok").and_then(|v| v.as_bool()).unwrap_or(false) {
        return Err(format!("engine.definitions rejected scene definition_ref='{}': {}", reference.canonical, validation));
    }
    Ok(())
}

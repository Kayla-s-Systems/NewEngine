...
pub fn build(params: &PrimitiveParams) -> PrimitiveMesh {
    ...
    for i in 0..=seg {
        ...
        vertices.push(PrimitiveVertex { pos: [ca * r, -hy, sa * r], nrm: [n.x, n.y, n.z], uv: [(i as f32)/(seg as f32), 0.0] });
    }
    let apex_index = vertices.len() as u32;
    vertices.push(PrimitiveVertex { pos: [0.0, hy, 0.0], nrm: [0.0, 1.0, 0.0], uv: [0.5, 1.0] });
    ...
    for i in 0..seg { indices.extend_from_slice(&[i, i + 1, apex_index]); }
    for i in 0..seg { let a = base_cap_center + 1 + i; let b = base_cap_center + 1 + i + 1; indices.extend_from_slice(&[base_cap_center, b, a]); }

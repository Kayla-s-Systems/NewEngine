...
pub fn build(params: &PrimitiveParams) -> PrimitiveMesh {
    ...
    for j in 0..=min {
        ...
        let ang_v = v * (2.0 * PI);
        let (sv, cv) = ang_v.sin_cos();
        for i in 0..=maj {
            let u = (i as f32) / (maj as f32);
            let ang_u = u * (2.0 * PI);
            let (su, cu) = ang_u.sin_cos();
            let x = (major_r + minor_r * cv) * cu;
            let y = minor_r * sv;
            let z = (major_r + minor_r * cv) * su;
            let n0 = Vec3::new(cu * cv, sv, su * cv);
            let n = n0 * (1.0 / n0.length().max(1.0e-20));
            vertices.push(PrimitiveVertex { pos: [x, y, z], nrm: [n.x, n.y, n.z], uv: [u, v] });
        }
    }
    let mut indices = Vec::with_capacity((maj * min * 2) as usize * 3);
    for j in 0..min { for i in 0..maj { let i0 = j * vert_w + i; let i1 = i0 + 1; let i2 = i0 + vert_w; let i3 = i2 + 1; indices.extend_from_slice(&[i0, i2, i1, i1, i2, i3]); } }
    PrimitiveMesh { vertices, indices, bounds_center: Vec3::ZERO, bounds_radius: major_r + minor_r }
}

...
pub fn build(params: &PrimitiveParams) -> PrimitiveMesh {
    ...
    vertices.push(PrimitiveVertex { pos: [0.0, 0.0, 0.0], nrm: [0.0, 1.0, 0.0], uv: [0.5, 0.5] });
    for i in 0..seg {
        let a = (i as f32) * (2.0 * PI) / (seg as f32);
        let (sa, ca) = a.sin_cos();
        vertices.push(PrimitiveVertex { pos: [ca * r, 0.0, sa * r], nrm: [0.0, 1.0, 0.0], uv: [ca * 0.5 + 0.5, sa * 0.5 + 0.5] });
    }
    let mut indices = Vec::with_capacity((seg as usize) * 3);
    for i in 0..seg { let a = 1 + i; let b = 1 + ((i + 1) % seg); indices.extend_from_slice(&[0, b, a]); }
    PrimitiveMesh { vertices, indices, bounds_center: Vec3::ZERO, bounds_radius: r }
}

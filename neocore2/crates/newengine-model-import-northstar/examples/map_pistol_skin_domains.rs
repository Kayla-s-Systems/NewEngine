...
fn dominant_joint(
    ...
    for (joint, weight) in joints
        .into_iter()
        .zip(weights)
        .chain(extra_joints.into_iter().zip(extra_weights))
    {
        if !weight.is_finite() || weight <= 0.0 {
            continue;
        }
        if best.is_none_or(|(best_weight, best_joint)| {
            weight > best_weight || (weight == best_weight && joint < best_joint)
        }) {
...
fn main() -> Result<(), String> {
    ...
    let common_path = args
        .next()
        .ok_or("usage: map_pistol_skin_domains COMMON.pak LOCAL1.pak [LOCAL2.pak ...]")?;
    ...
    for mesh in &common_geometry.meshes {
        ...
        for (vertex_index, (vertex, skin)) in mesh.vertices.iter().zip(skin).enumerate() {
            ...
            common_positions
                .entry(key(vertex.position, quantum))
                .or_default()
                .push((joint, mesh.name.clone(), vertex_index));
        }
    }

    println!(
        "common='{common_path}' meshes={} unique_positions={} quantum={quantum}",
        common_geometry.meshes.len(),
        common_positions.len()
    );
    ...
    for local_path in &local_paths {
        ...
        for mesh in &geometry.meshes {
            ...
            for (vertex, skin) in mesh.vertices.iter().zip(skin) {
                let Some(local_joint) = dominant_joint(
                    skin.joints,
                    skin.weights,
                    skin.joints_extra,
                    skin.weights_extra,
                ...
                *local_joint_totals.entry(local_joint).or_default() += 1;
                let Some(common_candidates) = common_positions.get(&key(vertex.position, quantum))
                else {
                    unmatched += 1;
                    continue;
                };
                let mut common_joints = common_candidates
                    .iter()
                    .map(|(joint, _, _)| *joint)
                    .collect::<Vec<_>>();
            ...
            println!(
                "  mesh='{}' vertices={} matched={mesh_matches}",
                mesh.name,
                mesh.vertices.len()
            );
        }
    ...
    println!(
        "summary exact_matches={exact_matches} ambiguous={ambiguous_matches} unmatched={unmatched}"
    );

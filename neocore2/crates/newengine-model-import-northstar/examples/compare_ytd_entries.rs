...
fn main() -> Result<(), String> {
    ...
    let read =
        |path: &str| -> Result<Vec<u8>, String> { fs::read(path).map_err(|e| e.to_string()) };
    let ab = read(&a)?;
    let bb = read(&b)?;
    ...
    if aa.entries().len() != ba.entries().len() {
    ...
    for meta in aa.entries() {
        let other = ba
            .entries()
            .iter()
            .find(|m| m.name.eq_ignore_ascii_case(&meta.name))
            .ok_or_else(|| format!("missing {}", meta.name))?;
        if meta.name.eq_ignore_ascii_case(&skip) {
            continue;
        }
        if meta.width != other.width
            || meta.height != other.height
            || meta.format != other.format
            || meta.color_space != other.color_space
            || meta.mip_count != other.mip_count
        {

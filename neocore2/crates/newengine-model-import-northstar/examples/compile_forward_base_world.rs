...
use std::{
    collections::{BTreeMap, BTreeSet}, env, fs, io::Write, path::{Path, PathBuf},
};
...
fn fnv1a64(text: &str) -> u64 {
    ...
    for b in text.as_bytes() { h ^= u64::from(*b); h = h.wrapping_mul(0x100000001b3); }
    h
}
...
fn material_identity(source: Option<&str>) -> String {
    let raw = source.unwrap_or("northstar/fallback/world").trim().replace('\\', "/").to_ascii_lowercase();
...
fn shadow_proxy(mesh_name: &str, material: &str) -> bool {
    let s = format!("{} {}", mesh_name.to_ascii_lowercase(), material.to_ascii_lowercase());
    ["shadowproxy", "shadow_proxy", "lib-shadowblocker"].iter().any(|token| s.contains(token))
}
fn cell_coord(center: [f32; 3], cell_size: f32) -> (i32, i32) {
    ((center[0] / cell_size).floor() as i32, (center[2] / cell_size).floor() as i32)
}
fn package_is_world_candidate(stem: &str) -> bool {
    let s = stem.to_ascii_lowercase();
    if s.starts_with("part-") || s.starts_with("part_") { return false; }
    if s.ends_with("-phys") || s.ends_with("-ingame") { return false; }
    !["script", "audio", "crowd", "anim", "cinematic"].iter().any(|t| s.contains(t))
}
...
fn encode_ydd(entry: YddBinaryEntry, output: &Path) -> Result<(), String> {
    let body = encode_ydd_binary_body(&YddBinaryDocument { entries: vec![entry] })?;
...
fn main() -> Result<(), String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--cell-size" => cell_size = args.next().ok_or("missing cell size")?.parse().map_err(|_| "invalid cell size")?,
            other => return Err(format!("unknown argument '{other}'")),
        }
    }
    if !cell_size.is_finite() || cell_size < 32.0 { return Err("cell size must be finite and >= 32".into()); }
    ...
    let mut pak_paths = fs::read_dir(&source_root).map_err(|e| e.to_string())?
        .filter_map(Result::ok).map(|e| e.path())
        .filter(|p| p.extension().and_then(|e| e.to_str()).is_some_and(|e| e.eq_ignore_ascii_case("pak")))
        .collect::<Vec<_>>();
    pak_paths.sort();
    ...
    for path in pak_paths {
        ...
        if !package_is_world_candidate(stem) { continue; }
        let bytes = match fs::read(&path) { Ok(v) => v, Err(e) => { error_rows.push(format!("{}\tread\t{}", path.display(), e)); continue; } };
        let pak = match PakFile::parse(bytes) { Ok(v) => v, Err(e) => { error_rows.push(format!("{}\tparse\t{}", path.display(), e.replace('\n', " "))); continue; } };
        if pak.resource("GEOMETRY_1").is_none() { continue; }
        let geometry = match decode_geometry_lod0(&pak) {
            Ok(v) => v,
            Err(e) => { error_rows.push(format!("{}\tgeometry\t{}", path.display(), e.replace('\n', " "))); continue; }
        };
        ...
        for mesh in geometry.meshes {
            ...
            if shadow_proxy(&mesh.name, &material) { continue; }
            let center = [
            ...
            let entry = material_rows.entry(material_hash.clone()).or_insert_with(|| (material.clone(), BTreeSet::new()));
            entry.1.insert(path.file_name().unwrap_or_default().to_string_lossy().to_string());
            let batch = cells.entry(coord).or_default().entry(material_hash).or_insert_with(|| Batch {
                source_material: material.clone(),
                bounds_min: [f32::INFINITY;3], bounds_max: [f32::NEG_INFINITY;3], ..Batch::default()
            });
            let base = u32::try_from(batch.vertices.len()).map_err(|_| "cell batch vertex address overflow")?;
            for mut v in mesh.vertices {
                v.position[0] -= origin[0]; v.position[1] -= origin[1]; v.position[2] -= origin[2];
                update_bounds(&mut batch.bounds_min, &mut batch.bounds_max, v.position);
                batch.vertices.push(v);
            }
            batch.indices.extend(mesh.indices.into_iter().map(|i| base + i));
            total_meshes += 1;
        }
        ...
        for ((cx,cz), batches) in cells {
            ...
            let mut entry_min = [f32::INFINITY;3]; let mut entry_max = [f32::NEG_INFINITY;3];
            let mut meshes = Vec::with_capacity(batches.len());
            let mut triangles = 0u64;
            for (material_hash, batch) in batches {
                triangles += (batch.indices.len()/3) as u64;
                for a in 0..3 { entry_min[a] = entry_min[a].min(batch.bounds_min[a]); entry_max[a] = entry_max[a].max(batch.bounds_max[a]); }
                meshes.push(YddBinaryMesh {
                    name: format!("mat_{material_hash}"),
                    material_ref: Some(format!("materials/forward_base/native/{material_hash}.nemat@mat_{material_hash}")),
            ...
            let origin_x = cx as f32 * cell_size; let origin_z = cz as f32 * cell_size;
            manifest_rows.push(format!(
                "{}\t{}\t{}\t{}\t{:.3}\t{:.3}\t{}\t{}\t{}\t{}",
                cx, cz, path.file_name().unwrap_or_default().to_string_lossy(), file_name,
                origin_x, origin_z, entry_name, triangles,
                if is_far_lod(stem) { "far_lod" } else { "streamed" }, output.display()
            ));
            total_triangles += triangles; total_outputs += 1;
        }
        println!("WORLD_PACKAGE_OK source='{}' outputs_so_far={} triangles_so_far={}", path.display(), total_outputs, total_triangles);
    }
    manifest_rows.sort();
    ...
    for row in manifest_rows { manifest_text.push_str(&row); manifest_text.push('\n'); }
    if let Some(parent)=manifest.parent(){fs::create_dir_all(parent).map_err(|e|e.to_string())?;}
    fs::write(&manifest,manifest_text).map_err(|e|e.to_string())?;
    let mut mat_text = String::from("material_hash\tsource_material\tpackages\n");
    for (hash,(source,packages)) in material_rows { mat_text.push_str(&format!("{}\t{}\t{}\n",hash,source,packages.into_iter().collect::<Vec<_>>().join(";"))); }

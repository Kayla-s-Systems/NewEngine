...
fn main() -> Result<(), String> {
    ...
    if selectors.is_empty() {
        return Err("no selectors".to_owned());
    }
    ...
    for selector in selectors {
        ...
        let matches = textures
            .iter()
            .filter(|t| t.source_path.to_ascii_lowercase().contains(&needle))
            .collect::<Vec<_>>();
        let texture = match matches.as_slice() {
            [t] => *t,
            [] => return Err(format!("no match {selector}")),
            many => {
                return Err(format!(
                    "ambiguous {selector}: {}",
                    many.iter()
        ...
        println!(
            "EXTRACT selector='{selector}' source='{}' {}x{} format={:?} output='{}'",
            texture.source_path,
            texture.width,
            texture.height,
            texture.format,

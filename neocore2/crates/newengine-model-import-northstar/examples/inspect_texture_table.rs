...
fn main() -> Result<(), String> {
    let source = env::args().nth(1).ok_or("usage: inspect_texture_table PAK")?;
    let pak = PakFile::parse(fs::read(&source).map_err(|e| e.to_string())?)?;
    for (ri, resource) in pak.resources().iter().enumerate() {
        if resource.kind != "TEXTURE_TABLE" && resource.kind != "VRAM_DESC_TABLE" { continue; }
        let payload = pak.resource_payload(resource)?;
        println!("RESOURCE {ri} kind={} abs=0x{:x} payload=0x{:x}", resource.kind, resource.absolute_offset, payload);
        for off in (0..512usize).step_by(8) {
            ...
            if let Some(ptr)=ptr {
                ...
                if let Ok(s)=pak.string_at(ptr) {
                    if !s.is_empty() && s.len()<220 && s.bytes().all(|b| b>=0x20 && b<0x7f) { note.push_str(&format!(" str={s:?}")); }
                }
            }

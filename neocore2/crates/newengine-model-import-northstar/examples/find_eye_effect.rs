...
fn main() -> Result<(), String> {
    ...
    for (ri, resource) in pak.resources().iter().enumerate() {
        ...
        for off in (0..0x1200usize).step_by(8) {
            ...
            if let Ok(Some(ptr)) = pak.resolve_pointer(payload + off) {
                if let Ok(s) = pak.string_at(ptr) {
                    ...
                    if low.contains("objects/characters/ellie/eyes")
                        || low.contains("g_eyewetness")
                        || low.contains("g_sclerabrightness")
                        || low.contains("g_ir")
                        || low.contains("g_pupilsize")
                    {
                        hits.push((off, ptr, s));
                    }
                }
        ...
        if !hits.is_empty() {
            println!(
                "EFFECT resource={} absolute=0x{:x} payload=0x{:x} hits={}",
                ri,
                resource.absolute_offset,
                payload,
                hits.len()
            );
            for (off, ptr, s) in hits {
                println!("  +0x{off:04x} -> 0x{ptr:x} {s:?}");
            }
            for off in (0..0x300usize).step_by(4) {
                if payload + off + 4 > pak.bytes().len() {
                    break;
                }

...
fn dump(pak: &PakFile, label: &str, at: usize, bytes: usize) {
    ...
    for off in (0..bytes).step_by(4) {
        ...
        let string = ptr.and_then(|p| pak.string_at(p).ok()).filter(|s| {
            !s.is_empty() && s.len() < 400 && s.bytes().all(|b| b.is_ascii_graphic() || b == b' ')
        });
...
fn main() -> Result<(), String> {
    ...
    let source = args
        .next()
        .ok_or("usage: inspect_geometry_links <pak> [instance-index]")?;
    ...
    let inst = pak
        .resolve_pointer(table_b + index * 8)?
        ...
        if let Some(p) = pak.resolve_pointer(inst + off).ok().flatten() {
            ...
            if name == "DEF" {
                for (sub, soff) in [("DEF40", 0x40usize), ("DEF30", 0x30usize)] {
                    if let Some(q) = pak.resolve_pointer(p + soff).ok().flatten() {
                        dump(&pak, sub, q, 0x100);
                    }
                }
            }
        }

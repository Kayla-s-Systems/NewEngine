...
fn dump_words(pak: &PakFile, at: usize, words: usize) -> String {
    (0..words)
        .map(|i| pak.read_u32(at + i * 4).map(|v| format!("{v:08x}")).unwrap_or_else(|_| "????????".into()))
        .collect::<Vec<_>>()
        .join(" ")
}
...
fn main() -> Result<(), String> {
    ...
    let source = args.next().ok_or("usage: inspect_vis_info <pak> [records]")?;
    let limit: usize = args.next().as_deref().unwrap_or("12").parse().map_err(|_| "invalid record count")?;
    ...
    let table = pak.resolve_pointer(base + 0x10)?.ok_or("VIS_INFO_1 record table pointer missing")?;
    ...
    for index in 0..count.min(limit) {
        ...
        let b = [
            pak.read_f32(rec + 0x18)?, pak.read_f32(rec + 0x1c)?, pak.read_f32(rec + 0x20)?,
            pak.read_f32(rec + 0x24)?, pak.read_f32(rec + 0x28)?, pak.read_f32(rec + 0x2c)?,
        ];
        ...
        for (label, ptr) in [("p08", p08), ("p10", p10), ("p38", p38)] {
            if let Some(ptr) = ptr {
                let string = pak.string_at(ptr).ok().filter(|s| !s.is_empty() && s.bytes().all(|b| b.is_ascii_graphic() || b == b' '));
                println!("  {label}=0x{ptr:x} words={} str={string:?}", dump_words(&pak, ptr, 12));
            } else {
                println!("  {label}=null");
            }
        }

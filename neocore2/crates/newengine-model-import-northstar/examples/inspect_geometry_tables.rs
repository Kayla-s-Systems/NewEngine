...
fn inspect_record(pak: &PakFile, label: &str, at: usize, size: usize) {
    ...
    for off in (0..size).step_by(4) {
        ...
        let string = ptr.and_then(|p| pak.string_at(p).ok()).filter(|s| !s.is_empty() && s.len() < 300 && s.bytes().all(|b| b.is_ascii_graphic() || b == b' '));
        if ptr.is_some() || string.is_some() || (f.is_finite() && f.abs() >= 0.00001 && f.abs() < 100000.0) || u < 100000 {
...
fn main() -> Result<(), String> {
    ...
    let source = args.next().ok_or("usage: inspect_geometry_tables <pak> [records]")?;
    let limit: usize = args.next().as_deref().unwrap_or("3").parse().map_err(|_| "invalid count")?;

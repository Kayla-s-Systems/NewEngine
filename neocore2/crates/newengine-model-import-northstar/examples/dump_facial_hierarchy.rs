...
fn main() -> Result<(), String> {
    let path = PathBuf::from(
        env::args()
            .nth(1)
            .ok_or("usage: dump_facial_hierarchy SKELETON.pak")?,
    );
    ...
    for (index, joint) in skeleton.joints.iter().enumerate() {
        ...
        while let Some(current) = cursor {
            ...
            depth += 1;
            if depth > skeleton.joints.len() {
                break;
            }
        }
        println!(
            "FACIAL index={} name='{}' parent={:?} parent_name={:?} under_headb={}",
            index,
            joint.name,
            joint.parent_index,
            joint
                .parent_index
                .map(|p| skeleton.joints[p as usize].name.as_str()),
            under_headb
        );
    }
    Ok(())

use std::{collections::{BTreeMap, HashMap, HashSet, VecDeque}, env, fs};
...
fn main() -> Result<(), String> {
    let path = env::args().nth(1).ok_or("usage: analyze_ydd_hair_cards FILE.ydd [logical]")?;
    ...
    for entry in doc.entries {
        println!("ENTRY '{}' source_to_model={:?}", entry.name, entry.skin_source_to_model);
        for (mesh_index, mesh) in entry.meshes.iter().enumerate() {
            ...
            for tri in 0..tri_count {
                ...
                for &v in &[a,b,c] {
                    if let Some(list) = vertex_tris.get_mut(v as usize) { list.push(tri); }
                }
            ...
            for seed in 0..tri_count {
                ...
                while let Some(t) = q.pop_front() {
                    count += 1;
                    let tri_vertices = [mesh.indices[t*3], mesh.indices[t*3+1], mesh.indices[t*3+2]];
            ...
            if let Some(skin) = mesh.skin.as_ref() {
                for vertex in skin {
                        ...
                        .chain(vertex.joints_extra.iter().zip(vertex.weights_extra.iter()))
                    {
                        ...
                        if weight > 0.0 { *weighted.entry(joint).or_default() += weight as f64; }
                    }
            ...
            weighted_sorted.sort_by(|a,b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));

...
fn main() -> Result<(), String> {
    let path = std::env::args().nth(1).map(PathBuf::from).ok_or("pak path required")?;
    ...
    let max_y = vertices.iter().map(|p| p[1]).fold(f32::NEG_INFINITY, f32::max);
    println!("meshes={} vertices={} y=[{:.6},{:.6}]", geometry.meshes.len(), vertices.len(), min_y, max_y);
    const BINS: usize = 24;
    for bin in 0..BINS {
        ...
        for p in &vertices {
            if p[1] >= y0 && (p[1] < y1 || bin+1==BINS) {
                count+=1; min_x=min_x.min(p[0]); max_x=max_x.max(p[0]); min_z=min_z.min(p[2]); max_z=max_z.max(p[2]);
            }
        }
        if count>0 {

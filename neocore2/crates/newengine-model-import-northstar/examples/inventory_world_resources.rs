...
fn main() -> Result<(), String> {
    let root = PathBuf::from(
        env::args()
            .nth(1)
            .ok_or("usage: inventory_world_resources <pak-dir>")?,
    );
    ...
    for path in paths {
        let bytes = match fs::read(&path) {
            Ok(v) => v,
            Err(e) => {
                println!("ERROR\t{}\tread\t{e}", path.display());
                continue;
            }
        };
        let pak = match PakFile::parse(bytes) {
            Ok(v) => v,
            Err(e) => {
                println!("ERROR\t{}\tparse\t{e}", path.display());
                continue;
            }
        };
        parsed += 1;
        ...
        for r in pak.resources() {
            *local.entry(r.kind.clone()).or_default() += 1;
            *kinds.entry(r.kind.clone()).or_default() += 1;
        }

impl SceneBridge {
    pub fn editor_scene_snapshots(
        ...
        for (entity, name) in world.query::<newengine_scene::components::Name>() {
            ...
            if world.get::<newengine_gameplay_world_runtime::gameplay::PlayerActor>(entity).is_some() {
            ...
            if world.get::<newengine_audio_api::AudioEmitter>(entity).is_some() {
                components.push("Audio Emitter".to_owned());
            }
            if world.get::<newengine_audio_api::AcousticSurface>(entity).is_some() {
                components.push("Acoustic Surface".to_owned());
            }
            if world.get::<newengine_audio_api::AudioEnvironmentZone>(entity).is_some() {
                components.push("Audio Environment Zone".to_owned());
            }
            if world.get::<newengine_audio_api::AudioPortal>(entity).is_some() {
                components.push("Audio Portal".to_owned());
            }
            if world.get::<newengine_audio_api::AudioAmbienceBed>(entity).is_some() {
            ...
            let kind = if world.get::<DirectionalLight>(entity).is_some() {
                ...
                "Spot Light"
            } else if world.get::<newengine_audio_api::AudioEnvironmentZone>(entity).is_some() {
                "Audio Environment Zone Actor"
            } else if world.get::<newengine_audio_api::AudioPortal>(entity).is_some() {
                "Audio Portal Actor"
            } else if world.get::<newengine_audio_api::AudioAmbienceBed>(entity).is_some() {
                "Audio Ambience Bed Actor"
            } else if world.get::<newengine_audio_api::AudioEmitter>(entity).is_some() {
                "Audio Emitter Actor"
            } else if world
                .get::<newengine_procedural_noise::ProceduralTerrain>(entity)
                .is_some()
            {
                "Terrain"
            } else if world.get::<newengine_gameplay_world_runtime::gameplay::PlayerActor>(entity).is_some() {
                "Player"

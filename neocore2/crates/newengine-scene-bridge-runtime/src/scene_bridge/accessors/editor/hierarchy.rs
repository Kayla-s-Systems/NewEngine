        ...
        .filter(|entity| {
            protected_root != Some(*entity)
                && world
                    ...
                    .is_none()
                && world.get::<newengine_gameplay_world_runtime::gameplay::PlayerActor>(*entity).is_none()
        })
        .collect::<Vec<_>>();
...
fn authored_editor_actor_root(world: &newengine_ecs::World, entity: EntityId) -> Option<EntityId> {
    ...
    while let Some(current) = cursor {
        if world.get::<newengine_gameplay_world_runtime::gameplay::PlayerActor>(current).is_some() {
            return Some(current);
        }

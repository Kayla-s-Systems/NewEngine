...
impl SceneBridge {
    ...
    pub fn authority_bridge(
        &self,
    ) -> std::sync::Arc<newengine_world_authority_runtime::RuntimeWorldAuthorityBridge> {
    ...
    fn refresh_selection_authority_and_inspector(&self, id: Option<EntityId>) {
        let authority = id.and_then(|entity| {
            let scene = self.scene.read();
            newengine_world_authority_runtime::current_entity_authority_map(scene.world())
    ...
    pub fn set_in_game_editor_enabled(&self, enabled: bool) -> bool {
        ...
        let _ = newengine_ui_client::set_surface_visible(GAME_HUD_SURFACE_ID, !enabled);
        ...
        if !enabled {
            ...
            *self.selection_authority.lock() = None;
            ...
            newengine_ui_client::set_surface_visible(EDITOR_INSPECTOR_SURFACE_ID, false);
        }

...
use newengine_gameplay_world_runtime::gameplay::{ModelRenderComponent, PreparedRenderMesh};
...
pub(crate) fn validate_scene_object_invariants(
    ...
    for (entity, _) in world.query::<Primitive>() {
        if world
            .get::<newengine_gameplay_world_runtime::gameplay::WorldItemVisualPart>(entity)
            .is_none()
        {
    ...
    for (entity, _) in world.query::<newengine_gameplay_world_runtime::gameplay::PlayerVisualPart>()
    {
        add_target(
            &mut targets,
            &mut target_indices,
    ...
    for (entity, anchor) in
        world.query::<newengine_gameplay_world_runtime::gameplay::SceneEntityAnchor>()
    {
    ...
    for target in targets {
        ...
        newengine_gameplay_world_runtime::gameplay::attach_scene_object_core(
            world,
            target.entity,
            position,
            half_extents,
...
fn scene_object_invariants_ui_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        ...
        if let Some(value) = newengine_runtime_env::var(OVERRIDE_ENV) {
            return matches!(
        ...
        let editor_shell_enabled = newengine_runtime_env::var(EDITOR_SHELL_ENV)
            .map(|value| {
                !matches!(
                    value.trim().to_ascii_lowercase().as_str(),
                    "0" | "false" | "no" | "off"
            ...
            .unwrap_or(true);
        let runtime_target =
            newengine_runtime_env::var("NEWENGINE_PLUGIN_TARGET").is_some_and(|value| {
                matches!(
                    value.trim().to_ascii_lowercase().as_str(),
...
fn add_target_by_anchor(
    ...
    target_indices: &mut FxHashMap<u64, usize>,
    entity: EntityId,
    role: newengine_gameplay_world_runtime::gameplay::SceneEntityRole,
) {
    use newengine_gameplay_world_runtime::gameplay::SceneEntityRole;

fn panel_component(panel: &UiScreenPanelDescriptor, visible: bool, hovered: bool) -> UiComponentNode {
    let mut component = UiComponentNode {
        ...
        icon: None,
        font_token: None,
        tone: if !visible { UiNodeTone::Disabled } else if hovered || panel.required { UiNodeTone::Accent } else { UiNodeTone::Normal },
        state_tags: {
            ...
            tags.push(if visible { "visible".to_owned() } else { "hidden".to_owned() });
            tags.push(if hovered { "hovered".to_owned() } else { "idle".to_owned() });
            tags
        },
    ...
    component.props.insert("visible".to_owned(), serde_json::json!(visible));
    component.props.insert("dockable".to_owned(), serde_json::json!(true));
    component.props.insert("resizable".to_owned(), serde_json::json!(true));
    component.props.insert("detachable".to_owned(), serde_json::json!(true));
    component.props.insert("tooltip".to_owned(), serde_json::json!(format!("{} · {}", panel.source_gateway, panel.data_contract)));
    component
}

...
pub(super) fn loading_overlay_components(
    ...
    background.props.insert("texture".to_owned(), serde_json::json!(STARTUP_BG_TEXTURE));
    background.props.insert("asset_path".to_owned(), serde_json::json!(STARTUP_BG_TEXTURE));
    background.props.insert("fit".to_owned(), serde_json::json!("cover"));
    background.props.insert("layer".to_owned(), serde_json::json!("background"));
    ...
    logo.props.insert("texture".to_owned(), serde_json::json!(STARTUP_LOGO_SPRITE));
    logo.props.insert("asset_path".to_owned(), serde_json::json!(STARTUP_LOGO_SPRITE));
    logo.props.insert("anchor".to_owned(), serde_json::json!("center"));
    logo.props.insert("sprite_columns".to_owned(), serde_json::json!(STARTUP_LOGO_SPRITE_COLUMNS));
    logo.props.insert("sprite_rows".to_owned(), serde_json::json!(STARTUP_LOGO_SPRITE_ROWS));
    logo.props.insert("frame_width".to_owned(), serde_json::json!(STARTUP_LOGO_FRAME_WIDTH));
    logo.props.insert("frame_height".to_owned(), serde_json::json!(STARTUP_LOGO_FRAME_HEIGHT));
    logo.props.insert("frame_count".to_owned(), serde_json::json!(STARTUP_LOGO_FRAME_COUNT));
    logo.props.insert("frame_index".to_owned(), serde_json::json!(logo_sprite_frame(frame_index)));
    logo.props.insert("fps".to_owned(), serde_json::json!(STARTUP_LOGO_FPS));
    logo.props.insert("loop".to_owned(), serde_json::json!(false));
    logo.props.insert("freeze_last_frame".to_owned(), serde_json::json!(true));
...
pub(super) fn error_overlay_components(status: &ScreenOverlayStatus) -> Vec<UiComponentNode> {
    ...
    detail.props.insert("selectable".to_owned(), serde_json::json!(true));

...
pub fn compile_xmlcentral_surface_root(xml: &str, document_ref: &str, style_ref: Option<&str>) -> Result<UiNodeRequest, String> {
    compile_request::validate_requested_entry(xml, "surface")?;
    let surface = parse_surface(xml).ok_or_else(|| format!("{document_ref}: .neui document has no <Surface> entry"))?;
    compile_surface_root(xml, &surface, document_ref, style_ref.or(surface.theme.as_deref()))
}

/// Decode packed `.neui` NEF8 bytes and compile its `@surface` entry into a provider-neutral UI tree.
pub fn compile_neui_bytes_surface_root(bytes: &[u8], document_ref: &str, style_ref: Option<&str>) -> Result<UiNodeRequest, String> {
    let (logical_path, _) = compile_request::split_ref(document_ref);
    let xml = compile_request::decode_neui_xmlcentral(if logical_path.trim().is_empty() { document_ref } else { &logical_path }, bytes)?;
    ...
    match method {
        assets_ui_method::COMPILE_DOCUMENT_V1 => {
            ...
            match compile_request::compile_document(state, request.clone()) {
                ...
                Err(e) => ok_json(compile_request::error_response_from_compile_error(e, &request)),
            }
        }
        ...
        assets_ui_method::MANIFEST_V1 | assets_ui_method::ENTRY_V1 | assets_ui_method::REGISTRY_V1 | assets_ui_method::BINDING_PLAN_V1 => {
            let request = serde_json::from_value::<AssetsUiRefRequest>(request_value).unwrap_or_default();
            match compile_request::compile_document(state, compile_request::compile_request_from_ref(request)) {
                Ok(response) => ok_json(response),
                Err(e) => ok_json(compile_request::error_response_from_message(e)),
            }
        }
...
pub fn assets_ui_gateway_service(client: AssetServiceClient) -> newengine_plugin_api::ServiceV1Dyn<'static> {
    ...
    JsonServiceRouter::with_state(ASSETS_UI_SERVICE_ID, AssetsUiRuntimeState::new(client))
        ...
        .post_json_result::<AssetsUiCompileRequest, AssetsUiCompileResponse, _>(assets_ui_method::COMPILE_DOCUMENT_V1, compile_request::compile_document)
        ...
        .post_json_result::<AssetsUiRefRequest, AssetsUiCompileResponse, _>(assets_ui_method::MANIFEST_V1, |state, request| compile_request::compile_document(state, compile_request::compile_request_from_ref(request)))
        .post_json_result::<AssetsUiRefRequest, AssetsUiCompileResponse, _>(assets_ui_method::ENTRY_V1, |state, request| compile_request::compile_document(state, compile_request::compile_request_from_ref(request)))
        .post_json_result::<AssetsUiRefRequest, AssetsUiCompileResponse, _>(assets_ui_method::REGISTRY_V1, |state, request| compile_request::compile_document(state, compile_request::compile_request_from_ref(request)))
        .post_json_result::<AssetsUiRefRequest, AssetsUiCompileResponse, _>(assets_ui_method::BINDING_PLAN_V1, |state, request| compile_request::compile_document(state, compile_request::compile_request_from_ref(request)))
...
"#;
    ...
    fn compiles_aurelia_asset_preview_stand_with_font_texture_and_binding_refs() {
        ...
        assert!(dependencies.iter().any(|dep| dep == "assets/loading/loading_ui.ytd@newengine_logo"));
        assert!(dependencies.iter().any(|dep| dep == "assets/ui/icons/builtin_icons.ytd@folder"));
        assert!(dependencies.iter().any(|dep| dep == "assets/ui/fonts/editor.neftd@tt_lakes_neue_trial_bold"));
        ...
        assert_eq!(surface.theme.as_deref(), Some("assets/ui/themes/northstar_editor.neui@editor_light"));

        let root = compile_surface_root(&xml, &surface, document_ref, surface.theme.as_deref()).unwrap();
        assert_eq!(root.kind, UiRuntimeNodeKind::Surface);
        assert_eq!(root.props.get("theme_ref").and_then(|value| value.as_str()), Some("assets/ui/themes/northstar_editor.neui@editor_light"));

        let title = find_node(&root, "preview.title").expect("title text node must compile");
        assert_eq!(title.kind, UiRuntimeNodeKind::Text);
        assert_eq!(title.font_token.as_deref(), Some("assets/ui/fonts/editor.neftd@tt_lakes_neue_trial_bold"));
        ...
        assert_eq!(logo.icon.as_deref(), Some("assets/loading/loading_ui.ytd@newengine_logo"));
        ...
        assert_eq!(folder.icon.as_deref(), Some("assets/ui/icons/builtin_icons.ytd@folder"));

        let caption = find_node(&root, "preview.caption").expect("caption text node must compile");
        assert_eq!(caption.font_token.as_deref(), Some("assets/ui/fonts/editor.neftd@tt_lakes_neue_trial_bold"));

        let binding_plan = parse_binding_plan(&xml, document_ref, &surface.name);
        assert_eq!(binding_plan.state_sources.len(), 1);
        assert!(binding_plan.bindings.iter().any(|binding| binding.element_id == "preview.logo" && binding.property == "texture"));
        assert!(binding_plan.bindings.iter().any(|binding| binding.element_id == "preview.caption" && binding.property == "text"));
        assert!(binding_plan.actions.iter().any(|action| action.action_id == "ui.preview.refresh" && action.target_gateway == "engine.ui"));
    }
}

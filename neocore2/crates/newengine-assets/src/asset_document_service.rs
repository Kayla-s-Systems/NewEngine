...
use newengine_assets_api::{
    asset_document_action_id, asset_edit_method, asset_inspect_method, file_type_method, AssetAccess, AssetDocument,
    AssetDocumentAction, AssetDocumentDiagnostic, AssetDocumentField,
    AssetDecodeRequest, AssetDocumentPreview, AssetDocumentRequest, AssetDocumentSection,
    AssetFileManifest, AssetFileTypeDescriptor, AssetFileTypeProbeRequest,
    AssetFileTypeProbeResult, AssetPatch, AssetPatchOperation, AssetPatchResult, ASSET_LIST_FILE_MANIFEST_OUTPUT,
    ASSETS_EDIT_BACKEND_CAPABILITY_ID, ASSETS_EDIT_SERVICE_ID, ASSETS_EDIT_SERVICE_METHODS,
    AssetService, ASSETS_INSPECT_BACKEND_CAPABILITY_ID, ASSETS_INSPECT_SERVICE_ID, ASSETS_INSPECT_SERVICE_METHODS,
    ENGINE_ASSET_TYPES_SERVICE_ID, ENGINE_ASSETS_EDIT_SERVICE_ID, ENGINE_ASSETS_INSPECT_SERVICE_ID,
};
...
use newengine_schema_api::{SchemaPatchDtoV1, SchemaPatchOperationV1, SchemaPropertyDescriptorV1, SchemaTransactionDtoV1, SchemaTypeDescriptorV1, SchemaValueKindV1, SCHEMA_RUNTIME_CONTRACT};
...
impl AssetInspectState {
    fn new(host: HostApiV1) -> Self { Self { host } }
    ...
    fn inspect_document(&self, request: AssetDocumentRequest) -> AssetDocument {
        ...
        if asset_ref.is_empty() {
            return AssetDocument {
                ...
                diagnostics: vec![AssetDocumentDiagnostic::warn("asset_ref.empty", "inspect_document_json_v1 requires asset_ref")],
        ...
        let descriptor = match descriptor_result {
            ...
            Err(error) => {
                diagnostics.push(AssetDocumentDiagnostic::warn("assets.types.unavailable", error));
                None
            }
        };
        ...
        let mut document = AssetDocument {
            ...
            document_kind: descriptor.as_ref().map(|d| d.asset_kind.clone()).unwrap_or_else(|| "asset_document".to_owned()),
            asset_kind: descriptor.as_ref().map(|d| d.asset_kind.clone()).unwrap_or_else(|| "asset".to_owned()),
            content_kind: descriptor.as_ref().and_then(|d| d.content_kind),
            semantic_gateway: descriptor.as_ref().map(|d| d.semantic_gateway.clone()).unwrap_or_else(|| "engine.assets".to_owned()),
            provider_service: descriptor.as_ref().map(|d| d.handler_service.clone()).unwrap_or_default(),
            inspect_contract: descriptor.as_ref().map(|d| d.inspect_contract.clone()).filter(|s| !s.is_empty()).unwrap_or_else(|| "asset.inspect.generic.v1".to_owned()),
            edit_contract: descriptor.as_ref().map(|d| d.edit_contract.clone()).unwrap_or_default(),
            ...
            preview: AssetDocumentPreview {
                kind: descriptor.as_ref().map(|d| d.asset_kind.clone()).unwrap_or_else(|| "asset".to_owned()),
                icon: icon_for_descriptor(descriptor.as_ref(), &extension).to_owned(),
                thumbnail_ref: String::new(),
                summary: descriptor.as_ref().map(|d| d.notes.clone()).unwrap_or_else(|| "generic VFS asset document".to_owned()),
            },
        ...
        document.sections.push(identity_section(&asset_ref, &logical_path, entry_selector.as_deref(), &extension));
        document.sections.push(type_descriptor_section(descriptor.as_ref()));
        document.sections.push(provider_contract_section(descriptor.as_ref()));
        document.sections.push(edit_policy_section(descriptor.as_ref()));
        ...
        self.hydrate_list_file_manifest_section(&logical_path, entry_selector.as_deref(), &mut document);
    ...
    fn resolve_descriptor(&self, asset_ref: &str) -> Result<AssetFileTypeProbeResult, String> {
        let payload = serde_json::to_vec(&AssetFileTypeProbeRequest { logical_path: asset_ref.to_owned() })
        ...
        let bytes = res.into_result().map(|v| v.into_vec()).map_err(|e| e.to_string())?;
        serde_json::from_slice::<AssetFileTypeProbeResult>(&bytes).map_err(|e| e.to_string())
    }


    fn hydrate_list_file_manifest_section(&self, logical_path: &str, selected_entry: Option<&str>, document: &mut AssetDocument) {
        let Some(descriptor) = document.descriptor.as_ref() else { return; };
        if !descriptor.outputs.iter().any(|output| output == ASSET_LIST_FILE_MANIFEST_OUTPUT) {
            return;
        }
        ...
        let bytes = match client.decode_v1(&request) {
            Ok(bytes) => bytes,
            Err(error) => {
                document.diagnostics.push(AssetDocumentDiagnostic::warn("listfile.manifest.unavailable", error));
                return;
            }
        };
        let manifest = match serde_json::from_slice::<AssetFileManifest>(&bytes) {
            ...
            Err(error) => {
                document.diagnostics.push(AssetDocumentDiagnostic::warn("listfile.manifest.invalid", format!("provider returned invalid AssetFileManifest: {error}")));
                return;
            }
        };
        ...
        let mut fields = vec![
            ...
            json_field("entry_count", "Entry Count", json!(manifest.entries.len()), false),
        ];
        if let Some(entry_name) = selected_entry {
            if let Some(entry) = manifest.entries.iter().find(|entry| entry.name == entry_name || entry.entry_ref.ends_with(&format!("@{entry_name}"))) {
                fields.push(string_field("selected_entry", "Selected Entry", &entry.entry_ref, false));
                fields.push(string_field("selected_kind", "Selected Kind", &entry.asset_kind, false));
                fields.push(string_field("selected_route", "Selected Route", &entry.route.gateway, false));
                fields.push(json_field("selected_dependencies", "Dependencies", json!(entry.dependencies), false));
            }
        }
        for (idx, entry) in manifest.entries.iter().take(12).enumerate() {
            fields.push(string_field(
                ...
                &format!("Entry {}", idx + 1),
                &format!("{} · {} · {}", entry.name, entry.asset_kind, entry.route.gateway),
                false,
            ));
        }
    ...
    fn hydrate_lifecycle_section(&self, logical_path: &str, document: &mut AssetDocument) {
        ...
        match client.uid_json_v1(logical_path) {
            ...
            Err(error) => document.diagnostics.push(AssetDocumentDiagnostic::warn("assets.uid.unavailable", error)),
        }
        match client.status_json_v1(logical_path) {
            Ok(value) => fields.push(json_field("status", "Status", value, false)),
            Err(error) => document.diagnostics.push(AssetDocumentDiagnostic::warn("assets.status.unavailable", error)),
        }
        match client.thumbnail_json_v1(json!({ "logical_path": logical_path })) {
            Ok(value) => {
                if let Some(label) = value.get("thumbnail").and_then(Value::as_str).or_else(|| value.get("cache_key").and_then(Value::as_str)) {
                    document.preview.thumbnail_ref = label.to_owned();
                }
                fields.push(json_field("thumbnail", "Preview", value, false));
            }
            Err(error) => document.diagnostics.push(AssetDocumentDiagnostic::warn("assets.thumbnail.unavailable", error)),
        }
        if !fields.is_empty() {
            document.sections.push(AssetDocumentSection { id: "lifecycle".to_owned(), title: "Lifecycle / import state".to_owned(), fields });
        }
    }
}
...
impl AssetEditState {
    ...
    fn validate_patch(&self, patch: AssetPatch) -> AssetPatchResult {
        let mut result = AssetPatchResult { asset_ref: normalize_asset_ref(&patch.asset_ref), ..AssetPatchResult::default() };
        if result.asset_ref.is_empty() {
            result.diagnostics.push(AssetDocumentDiagnostic::error("asset_ref.empty", "patch requires asset_ref"));
            return result;
        }
        if let Some(schema_patch) = patch.schema_patch.as_ref() {
            if schema_patch.target_ref.trim().is_empty() || schema_patch.target_ref != result.asset_ref {
        ...
        if patch.operations.is_empty() {
            ...
            result.diagnostics.push(AssetDocumentDiagnostic::info("patch.empty", "empty patch is valid and has no write effect"));
            return result;
        }
        if patch.edit_contract.trim().is_empty() || patch.edit_contract == newengine_assets_api::ASSETS_EDIT_RUNTIME_CONTRACT {
            result.diagnostics.push(AssetDocumentDiagnostic::warn(
        ...
        result.accepted = true;
        result.dirty = true;
        result.diagnostics.push(AssetDocumentDiagnostic::info("patch.accepted", "patch transport is valid; provider-specific writer owns final validation"));
        result
    }
    ...
    fn apply_patch(&self, patch: AssetPatch) -> AssetPatchResult {
        ...
        let operation = match first_op.op.trim().to_ascii_lowercase().as_str() {
            ...
            _ => {
                result.accepted = false;
                result.diagnostics.push(AssetDocumentDiagnostic::error("patch.operation.unsupported", format!("unsupported asset patch operation '{}'", first_op.op)));
                return result;
            }
        };
        ...
        match client.list_file_repack_json_v1(payload) {
            Ok(value) => {
                ...
                let applied = value.get("applied").and_then(Value::as_bool).unwrap_or(false);
                result.accepted = ok;
                result.written = applied;
                result.dirty = !applied;
                result.diagnostics.push(AssetDocumentDiagnostic::info(
                    "listfile.repack",
                    value.get("message").and_then(Value::as_str).unwrap_or("NEF8 ListFile writer completed"),
                ));
                if !ok {
                    result.diagnostics.push(AssetDocumentDiagnostic::warn("listfile.repack.not_applied", "writer rejected or dry-ran the patch"));
                }
                result
            }
            Err(error) => {
                ...
                result.written = false;
                result.dirty = true;
                result.diagnostics.push(AssetDocumentDiagnostic::error("listfile.repack.failed", error));
                result
            }
        }
    }
...
fn asset_document_actions(document: &AssetDocument, selected_entry: Option<&str>) -> Vec<AssetDocumentAction> {
    ...
    let delete_patch = if can_write && has_entry_selection {
        ...
        Some(AssetPatch {
            ...
            schema_patch: Some(schema_patch_for_document(document, &target_ref, "asset-delete", vec![schema_op.clone()])),
            transaction: Some(schema_transaction_for_document(document, &target_ref, "asset-delete", vec![schema_op], Vec::new())),
            ..AssetPatch::default()
        })
    } else {
        None
    ...
    actions.push(AssetDocumentAction {
        ...
        patch_template: if can_write && document.dirty {
            Some(AssetPatch {
                ...
                requester: "ui.assets.catalog".to_owned(),
                operations: Vec::new(),
                schema_patch: Some(schema_patch_for_document(document, &target_ref, "asset-save", Vec::new())),
                transaction: Some(schema_transaction_for_document(document, &target_ref, "asset-save", Vec::new(), Vec::new())),
                ..AssetPatch::default()
            })
        } else {
            None
...
fn identity_section(asset_ref: &str, logical_path: &str, entry_selector: Option<&str>, extension: &str) -> AssetDocumentSection {
    AssetDocumentSection {
        ...
        fields: vec![
            ...
            string_field("entry_selector", "Entry Selector", entry_selector.unwrap_or("<whole file>"), false),
...
fn type_descriptor_section(descriptor: Option<&AssetFileTypeDescriptor>) -> AssetDocumentSection {
    ...
    AssetDocumentSection {
        ...
        fields: vec![
            ...
            string_field("semantic_gateway", "Semantic Gateway", &desc.semantic_gateway, false),
            string_field("selector_syntax", "Selector Syntax", desc.selector_syntax.as_deref().unwrap_or("<none>"), false),
            json_field("content_kind", "Content Kind", json!(desc.content_kind), false),
            json_field("outputs", "Outputs", json!(desc.outputs), false),
            json_field("consumer_domains", "Consumers", json!(desc.consumer_domains), false),
        ],
    }
}
...
fn provider_contract_section(descriptor: Option<&AssetFileTypeDescriptor>) -> AssetDocumentSection {
    let Some(desc) = descriptor else {
        return AssetDocumentSection {
            ...
            fields: vec![string_field("contract", "Contract", "unregistered type", false)],
        };
    };
    AssetDocumentSection {
        id: "provider_contract".to_owned(),
        title: "Provider contracts".to_owned(),
        fields: vec![
            string_field("handler_service", "Handler Service", &desc.handler_service, false),
            string_field("read_method", "Read Method", &desc.read_method, false),
            string_field("inspect_contract", "Inspect Contract", &desc.inspect_contract, false),
            string_field("edit_contract", "Edit Contract", &desc.edit_contract, false),
            json_field("preview_provider", "Preview Provider", json!(desc.preview_provider), false),
            json_field("runtime_ready", "Runtime Ready", json!(desc.runtime_ready), false),
        ],
    }
}

fn editable_schema_section(title: &str, descriptor: Option<&AssetFileTypeDescriptor>, can_apply_patch: bool) -> AssetDocumentSection {
    let document_kind = descriptor.map(|d| d.asset_kind.as_str()).unwrap_or("asset_document");
    let mut fields = vec![
        schema_field("display_name", "Display Name", json!(title), SchemaValueKindV1::String, true),
        schema_field("editor_tags", "Editor Tags", json!([]), SchemaValueKindV1::StringList, true),
        schema_field("import_notes", "Import Notes", json!(""), SchemaValueKindV1::String, true),
        string_field("document_kind", "Document Kind", document_kind, false),
        json_field("save_enabled", "Save Enabled", json!(can_apply_patch), false),
    ];
    if !can_apply_patch {
        fields.push(string_field("save_blocked_by", "Save Blocked By", "missing format/package writer capability", false));
    }
    AssetDocumentSection {
...
fn edit_policy_section(descriptor: Option<&AssetFileTypeDescriptor>) -> AssetDocumentSection {
    ...
    let can_apply_patch = descriptor
        .map(|d| d.write_back_available && !d.writer_capability.trim().is_empty())
        .unwrap_or(false);
    let writer_capability = descriptor.map(|d| d.writer_capability.as_str()).unwrap_or("");
    AssetDocumentSection {
        id: "edit_policy".to_owned(),
        title: "Edit policy".to_owned(),
        fields: vec![
            json_field("schema_editable", "Schema Editable", json!(schema_editable), false),
            json_field("can_apply_patch", "Can Apply Patch", json!(can_apply_patch), false),
            string_field("patch_route", "Patch Route", ENGINE_ASSETS_EDIT_SERVICE_ID, false),
            string_field("writer_capability", "Writer Capability", if writer_capability.is_empty() { "missing" } else { writer_capability }, false),
            string_field("write_owner", "Write Owner", if can_apply_patch { "format/package writer provider" } else { "missing format writer provider" }, false),
        ],
    }
}
...
fn schema_field(id: &str, label: &str, value: Value, value_kind: SchemaValueKindV1, editable: bool) -> AssetDocumentField {
    AssetDocumentField {
        id: id.to_owned(),
...
fn schema_property(id: &str, label: &str, value_kind: SchemaValueKindV1, value: Value, editable: bool) -> SchemaPropertyDescriptorV1 {
    ...
    property.tags.push(if editable { "editable".to_owned() } else { "readonly".to_owned() });
    property
}
...
fn asset_document_schema_type(document: &AssetDocument) -> SchemaTypeDescriptorV1 {
    ...
    SchemaTypeDescriptorV1 {
        type_id: format!("newengine.assets.document.{}", document.asset_kind.replace(' ', "_").replace('/', "_")),
...
fn asset_action_input_schema(type_id: &str, display_name: &str, properties: Vec<SchemaPropertyDescriptorV1>) -> SchemaTypeDescriptorV1 {
    SchemaTypeDescriptorV1 {
...
fn schema_patch_for_document(document: &AssetDocument, target_ref: &str, reason: &str, operations: Vec<SchemaPatchOperationV1>) -> SchemaPatchDtoV1 {
    SchemaPatchDtoV1 {
        target_type: document.schema_type.as_ref().map(|schema| schema.type_id.clone()).unwrap_or_else(|| document.document_kind.clone()),
...
fn icon_for_descriptor(descriptor: Option<&AssetFileTypeDescriptor>, extension: &str) -> &'static str {
    match descriptor.map(|d| d.asset_kind.as_str()).unwrap_or(extension) {
...
fn normalize_asset_ref(value: &str) -> String {
    ...
    while let Some(rest) = out.strip_prefix("./") { out = rest.to_owned(); }
...
fn split_entry_ref(asset_ref: &str) -> (String, Option<String>) {
    ...
    let entry = parts.next().map(str::trim).filter(|s| !s.is_empty()).map(ToOwned::to_owned);
    (path, entry)
}

fn path_extension(path: &str) -> String {
    path.rsplit('/').next().unwrap_or(path).rsplit_once('.').map(|(_, ext)| ext.to_ascii_lowercase()).unwrap_or_default()
}

fn file_name(path: &str) -> String {
    path.rsplit('/').next().filter(|s| !s.is_empty()).unwrap_or(path).to_owned()
}
...
fn inspect_invoke_json(state: &mut AssetInspectState, payload: Blob) -> RResult<Blob, RString> {
    let envelope = match serde_json::from_slice::<InvokeEnvelope>(payload.as_slice()) {
        ...
        Err(e) => return RResult::RErr(RString::from(format!("assets.inspect: invalid invoke_json payload: {e}"))),
    };
    match envelope.method.as_str() {
        asset_inspect_method::INSPECT_DOCUMENT_JSON_V1 | asset_inspect_method::PREVIEW_JSON_V1 | asset_inspect_method::VALIDATE_REF_JSON_V1 => {
            let request = match serde_json::from_value::<AssetDocumentRequest>(envelope.request) {
                Ok(request) => request,
                Err(e) => return RResult::RErr(RString::from(format!("assets.inspect: invalid document request: {e}"))),
            };
            ok_json(state.inspect_document(request))
        }
        other => RResult::RErr(RString::from(format!("assets.inspect: unknown invoke method '{other}'"))),
    }
}
...
fn edit_invoke_json(state: &mut AssetEditState, payload: Blob) -> RResult<Blob, RString> {
    let envelope = match serde_json::from_slice::<InvokeEnvelope>(payload.as_slice()) {
        Ok(envelope) => envelope,
        Err(e) => return RResult::RErr(RString::from(format!("assets.edit: invalid invoke_json payload: {e}"))),
    };
    match envelope.method.as_str() {
        asset_edit_method::VALIDATE_PATCH_JSON_V1 => {
            let patch = match serde_json::from_value::<AssetPatch>(envelope.request) {
                Ok(patch) => patch,
                Err(e) => return RResult::RErr(RString::from(format!("assets.edit: invalid patch: {e}"))),
            };
        ...
        asset_edit_method::DIRTY_STATE_JSON_V1 => ok_json(AssetPatchResult {
            accepted: true,
            diagnostics: vec![AssetDocumentDiagnostic::info("dirty_state.generic", "generic edit provider has no local dirty cache")],
            ..AssetPatchResult::default()
        }),
        other => RResult::RErr(RString::from(format!("assets.edit: unknown invoke method '{other}'"))),
    }
}

pub fn asset_document_inspect_gateway_service(host: HostApiV1) -> newengine_plugin_api::ServiceV1Dyn<'static> {
    ...
    JsonServiceRouter::with_state(ASSETS_INSPECT_SERVICE_ID, AssetInspectState::new(host))
        ...
        .blob(asset_inspect_method::SHUTDOWN_V1, |_state, _payload| ok_empty_blob())
        .into_service_v1()
}

pub fn asset_document_edit_gateway_service(host: HostApiV1) -> newengine_plugin_api::ServiceV1Dyn<'static> {
    let description = engine_gateway_provider_service_description(
    ...
    JsonServiceRouter::with_state(ASSETS_EDIT_SERVICE_ID, AssetEditState::new(host))
        ...
        .blob(asset_edit_method::SHUTDOWN_V1, |_state, _payload| ok_empty_blob())
        .into_service_v1()
}

pub fn register_asset_document_gateways_best_effort(host: HostApiV1) -> bool {
    let inspect_ok = register_engine_gateway_provider_service_dynamic_best_effort(EngineGatewayProviderDeclDynamic {
        gateway: ENGINE_ASSETS_INSPECT_SERVICE_ID,
    ...
    let edit_ok = register_engine_gateway_provider_service_dynamic_best_effort(EngineGatewayProviderDeclDynamic {
        gateway: ENGINE_ASSETS_EDIT_SERVICE_ID,

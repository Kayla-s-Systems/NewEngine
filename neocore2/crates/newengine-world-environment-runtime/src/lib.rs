...
pub const WORLD_ENVIRONMENT_GATEWAY_OWNER: &str =
    "newengine-world-environment-runtime.environment-gateway";
...
impl EnvironmentProviderState {
    ...
    fn sample_at_position_json_v1(
        &self,
        req: EnvironmentSampleAtPositionRequest,
    ...
    fn preview_time_json_v1(&mut self, req: EnvironmentPreviewTimeRequest) -> EnvironmentFrameDto {
        ...
        frame_req.time.game.seconds_of_day =
            frame_req.time.game.normalized_day * frame_req.time.game.seconds_per_game_day.max(1.0);
    ...
    fn invoke_json(&mut self, payload: Blob) -> RResult<Blob, RString> {
        let req = match payload_json(&payload).and_then(|v| {
            serde_json::from_value::<EnvironmentInvokeRequest>(v).map_err(|e| e.to_string())
        }) {
            Ok(v) => v,
        ...
        match req.method.as_str() {
            WORLD_ENVIRONMENT_SERVICE_METHOD_FRAME_JSON_V1 => {
                let req = match payload_json(&payload).and_then(|v| {
                    serde_json::from_value::<EnvironmentFrameRequest>(v).map_err(|e| e.to_string())
                }) {
                    Ok(v) => v,
                    Err(e) => return RResult::RErr(RString::from(e)),
                };
            ...
            WORLD_ENVIRONMENT_SERVICE_METHOD_SAMPLE_AT_POSITION_JSON_V1 => {
                let req = match payload_json(&payload).and_then(|v| {
                    serde_json::from_value::<EnvironmentSampleAtPositionRequest>(v)
            ...
            WORLD_ENVIRONMENT_SERVICE_METHOD_SNAPSHOT_JSON_V1 => {
                let req = match payload_json(&payload).and_then(|v| {
                    serde_json::from_value::<EnvironmentSnapshotRequest>(v)
                        .map_err(|e| e.to_string())
                }) {
                    Ok(v) => v,
                    Err(e) => return RResult::RErr(RString::from(e)),
            ...
            WORLD_ENVIRONMENT_SERVICE_METHOD_RESTORE_JSON_V1 => {
                let req = match payload_json(&payload).and_then(|v| {
                    serde_json::from_value::<EnvironmentRestoreRequest>(v)
                        .map_err(|e| e.to_string())
                }) {
                    Ok(v) => v,
                    Err(e) => return RResult::RErr(RString::from(e)),
            ...
            WORLD_ENVIRONMENT_SERVICE_METHOD_PREVIEW_TIME_JSON_V1 => {
                let req = match payload_json(&payload).and_then(|v| {
                    serde_json::from_value::<EnvironmentPreviewTimeRequest>(v)
                        .map_err(|e| e.to_string())
                }) {
                    Ok(v) => v,
                    Err(e) => return RResult::RErr(RString::from(e)),
...
fn environment_gateway_service(
    ...
    .post_json(
        WORLD_ENVIRONMENT_SERVICE_METHOD_FRAME_JSON_V1,
        |state, req: EnvironmentFrameRequest| state.frame_json_v1(req),
    )
    .post_json(
        WORLD_ENVIRONMENT_SERVICE_METHOD_SAMPLE_AT_POSITION_JSON_V1,
    ...
    .post_json(
        WORLD_ENVIRONMENT_SERVICE_METHOD_SNAPSHOT_JSON_V1,
        |state, req: EnvironmentSnapshotRequest| state.snapshot_json_v1(req),
    )
    .post_json(
        WORLD_ENVIRONMENT_SERVICE_METHOD_RESTORE_JSON_V1,
        |state, req: EnvironmentRestoreRequest| state.restore_json_v1(req),
    )
    .post_json(
        WORLD_ENVIRONMENT_SERVICE_METHOD_PREVIEW_TIME_JSON_V1,
        |state, req: EnvironmentPreviewTimeRequest| state.preview_time_json_v1(req),
    )
    .blob(
        WORLD_ENVIRONMENT_SERVICE_METHOD_SHUTDOWN_V1,

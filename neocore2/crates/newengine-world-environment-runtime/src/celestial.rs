...
use newengine_world_environment_api::{CelestialBodyDto, Color3Dto, TimeOfDayPhase, TimeOfDayStateDto, Vec3Dto};
...
pub(crate) fn time_of_day_state(tod: f32) -> TimeOfDayStateDto {
    ...
    let phase = PHASES
        ...
        .map(|(phase, _, _)| *phase)
        .unwrap_or_else(|| if day_blend >= 0.50 { TimeOfDayPhase::Day } else { TimeOfDayPhase::Night });
    TimeOfDayStateDto { normalized_day: tod, hours, phase, dawn_dusk_blend, day_blend, night_blend }
}
...
pub(crate) fn sun_body(tod: f32) -> CelestialBodyDto {
    ...
    let direction = normalize(Vec3Dto::new(orbit.cos(), altitude_raw, (tau * tod).sin() * 0.35));
    CelestialBodyDto {
        direction_world: direction,
        altitude_radians: altitude_raw.asin(),
        azimuth_radians: tau * tod,
        angular_radius_radians: 0.00465,
        color_linear: mix_color_local(Color3Dto::new(1.0, 0.47, 0.22), Color3Dto::new(1.0, 0.95, 0.82), visibility),

...
use newengine_authored_world_runtime::AuthoredGroundPlacementSurface;
...
fn static_ground_height(world: &newengine_ecs::World, x: f32, z: f32) -> Option<f32> {
    ...
    for (entity, _) in world.query::<AuthoredGroundPlacementSurface>() {
        let Some(collider) =
...
fn has_static_ground_surfaces(world: &newengine_ecs::World) -> bool {
    world
        .query::<AuthoredGroundPlacementSurface>()
        .next()
        .is_some()
}

pub fn terrain_height(world: &newengine_ecs::World, terrain: EntityId, x: f32, z: f32) -> f32 {
    let origin = world
...
pub(super) fn effective_foliage_spec(spec: &AuthoredFoliageSpec) -> AuthoredFoliageSpec {
    let mut effective = spec.clone();
    let stress = newengine_runtime_env::var("NEWENGINE_FOLIAGE_STRESS")
        .map(|value| {
            matches!(
...
pub(super) fn collect_tree_placements(
    ...
    let (terrain_half_x, terrain_half_z, domain_source) = if let Some(surface) = terrain_surface {
        ...
        newengine_ulog_api::ulog::warn!(
            "authored-environment foliage placement rejected: terrain={:?} has no bootstrap sampler and no ECS terrain surface",
            terrain
        );
        return Vec::new();
    };
    ...
    if !half_x.is_finite() || !half_z.is_finite() || half_x <= 0.5 || half_z <= 0.5 {
        newengine_ulog_api::ulog::warn!(
            "authored-environment foliage placement rejected: terrain={:?} domain={} half=({:.3},{:.3}) edge_margin={:.3} origin=({:.2},{:.2},{:.2})",
            terrain,
            domain_source,
            terrain_half_x,
            terrain_half_z,
        ...
        for gx in spec.grid_min..=spec.grid_max {
            if placements.len() as u32 >= spec.max_count {
                newengine_ulog_api::ulog::info!(
                    "authored-environment foliage placement domain: terrain={:?} domain={} origin=({:.2},{:.2},{:.2}) half=({:.2},{:.2}) placed={} gate_rejected={} edge_rejected={} player_rejected={} density_gate={:.3}",
                    terrain,
                    domain_source,
                    origin.x,
                    origin.y,
    ...
    newengine_ulog_api::ulog::info!(
        "authored-environment foliage ground projection: mode='{}' placed={} ground_rejected={} surface_offset={:.3} policy='confirmed surface hit required; no origin-y fallback'",

...
pub(super) fn material_for_slot(
    ...
    if let Some(reference) = material_ref.filter(|value| is_nemat_entry_ref(value)) {
        if let Some(mut response) = load_authored_material_descriptor_asset(reference) {
        ...
        newengine_ulog_api::ulog::warn!(
            "authored-environment foliage material exact selector unresolved ref='{}' slot='{}'; using role fallback",
            reference,
            slot,
        );
    }

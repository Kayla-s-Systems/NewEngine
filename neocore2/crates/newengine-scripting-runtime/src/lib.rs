...
use newengine_scripting_api::{
    ...
    ScriptingModuleRef, ScriptingModuleUnloadRequest, ScriptingRequestBytes,
    ...
    SCRIPTING_SERVICE_METHOD_INFO, SCRIPTING_SERVICE_METHOD_INVOKE,
    SCRIPTING_SERVICE_METHOD_INVOKE_BYTES_V1, SCRIPTING_SERVICE_METHOD_LOAD_MODULE_BYTES_V1,
    SCRIPTING_SERVICE_METHOD_SHUTDOWN_V1, SCRIPTING_SERVICE_METHOD_UNLOAD_MODULE_JSON_V1,
...
impl ScriptingRuntimeState {
    ...
    pub fn runtime_info(&self) -> ScriptingRuntimeServiceInfo {
        ScriptingRuntimeServiceInfo {
            ...
            methods: newengine_scripting_api::SCRIPTING_SERVICE_METHODS
                .iter()
                .map(|it| (*it).to_owned())
                .collect(),
    ...
    fn invoke_control_json(&mut self, payload: Blob) -> RResult<Blob, RString> {
        let envelope = match serde_json::from_slice::<ScriptingInvokeEnvelope>(payload.as_slice()) {
            ...
            Err(e) => {
                return RResult::RErr(RString::from(format!(
                    "scripting.api: invalid invoke envelope: {e}"
                )))
            }
        };
        ...
        match envelope.method.as_str() {
            SCRIPTING_SERVICE_METHOD_INVOKE_BYTES_V1 => {
                self.invoke_request_payload(envelope.request_bytes, false)
            }
            SCRIPTING_SERVICE_METHOD_FRAME_BYTES_V1 => {
                self.invoke_request_payload(envelope.request_bytes, true)
            }
            SCRIPTING_SERVICE_METHOD_LOAD_MODULE_BYTES_V1 => {
                self.load_module_payload(envelope.request_bytes)
            }
    ...
    fn load_module_payload(&mut self, payload: Vec<u8>) -> RResult<Blob, RString> {
        let request = match decode_scripting_module_load_bytes_request(&payload) {
            ...
            Err(e) => {
                return RResult::RErr(RString::from(format!(
                    "scripting.api: invalid binary module load envelope: {e}"
                )))
            }
        };
        RResult::ROk(Blob::from(encode_scripting_module_load_bytes_response(
            &self.load_module_bytes(request),
        )))
    }
    ...
    fn invoke_request_payload(&mut self, payload: Vec<u8>, frame: bool) -> RResult<Blob, RString> {
        let request = match decode_scripting_request_bytes(&payload) {
            ...
            Err(e) => {
                return RResult::RErr(RString::from(format!(
                    "scripting.api: invalid binary scripting request envelope: {e}"
                )))
            }
        };
        let response = if frame {
            self.frame_bytes(request)
        } else {
            self.invoke_bytes(request)
        };
...
fn decode_module_load_blob(payload: Blob) -> RResult<ScriptingModuleLoadBytesRequest, RString> {
    match decode_scripting_module_load_bytes_request(payload.as_slice()) {
        ...
        Err(e) => RResult::RErr(RString::from(format!(
            "scripting.api: invalid binary module load payload: {e}"
        ))),
    }
}
...
fn decode_request_blob(payload: Blob) -> RResult<ScriptingRequestBytes, RString> {
    match decode_scripting_request_bytes(payload.as_slice()) {
        Ok(request) => RResult::ROk(request),
        Err(e) => RResult::RErr(RString::from(format!(
            "scripting.api: invalid binary scripting request payload: {e}"
        ))),
    }
}
...
pub fn scripting_gateway_service() -> newengine_plugin_api::ServiceV1Dyn<'static> {
    ...
    JsonServiceRouter::with_state(SCRIPTING_SERVICE_ID, ScriptingRuntimeState::default())
        ...
        .blob(
            SCRIPTING_SERVICE_METHOD_LOAD_MODULE_BYTES_V1,
            |state, payload| match decode_module_load_blob(payload) {
                RResult::ROk(request) => RResult::ROk(Blob::from(
                    encode_scripting_module_load_bytes_response(&state.load_module_bytes(request)),
                )),
        ...
        .blob(
            SCRIPTING_SERVICE_METHOD_INVOKE_BYTES_V1,
            |state, payload| match decode_request_blob(payload) {
                RResult::ROk(request) => RResult::ROk(Blob::from(encode_scripting_response_bytes(
                    &state.invoke_bytes(request),
                ))),
                RResult::RErr(err) => RResult::RErr(err),
            },
        )
        .blob(
            ...
            |state, payload| match decode_request_blob(payload) {
                RResult::ROk(request) => RResult::ROk(Blob::from(encode_scripting_response_bytes(
                    &state.frame_bytes(request),
                ))),
                RResult::RErr(err) => RResult::RErr(err),
            },
        )
        .blob(SCRIPTING_SERVICE_METHOD_INVOKE, |state, payload| {
            state.invoke_control_json(payload)
        })
...
mod tests {
    ...
    fn baseline_runtime_returns_empty_opaque_response() {
        ...
        let out = state.invoke_bytes(ScriptingRequestBytes {
    ...
    fn binary_wire_methods_accept_binary_envelopes() {
        ...
        assert!(matches!(
            decode_module_load_blob(Blob::from(encode_scripting_module_load_bytes_request(
                &load
            ))),
            RResult::ROk(_)
        ));
        ...
        assert!(matches!(
            decode_request_blob(Blob::from(encode_scripting_request_bytes(&request))),
            RResult::ROk(_)
        ));
    }
}

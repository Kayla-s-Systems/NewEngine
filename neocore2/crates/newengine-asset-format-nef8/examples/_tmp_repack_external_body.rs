...
use newengine_assets_api::{decode_list_file_envelope, encode_list_file, parse_list_file_header, ListFileEncodeRequest};
...
fn replace_all(input: Vec<u8>, from: &[u8], to: &[u8]) -> Vec<u8> {
    ...
    let mut i = 0usize;
    while i < input.len() {
        if input[i..].starts_with(from) { out.extend_from_slice(to); i += from.len(); }
...
fn forbidden(bytes: &[u8]) -> bool {
    ...
    [b"naughtydog".as_slice(), b"tlou2".as_slice(), b"northstar.northstar".as_slice()]
...
fn repack(asset: &Path, body_path: &Path) -> Result<(), String> {
    ...
    let body = fs::read(body_path).map_err(|e| format!("read body {}: {e}", body_path.display()))?;
    if forbidden(&body) { return Err(format!("forbidden namespace remains in supplied body: {}", asset.display())); }
    ...
    if forbidden(&metadata) { return Err(format!("forbidden namespace remains in metadata: {}", asset.display())); }
    let mut encoder = DeflateEncoder::new(Vec::new(), Compression::fast());
    encoder.write_all(&body).map_err(|e| format!("deflate {}: {e}", asset.display()))?;
    let stored = encoder.finish().map_err(|e| format!("deflate finish {}: {e}", asset.display()))?;
    ...
    let verify = decode_list_file_envelope(&rebuilt, header.content_kind, &asset.to_string_lossy())?;
    if verify.body != body { return Err(format!("body verification mismatch: {}", asset.display())); }
    let vm = serde_json::to_vec(&verify.metadata).map_err(|e| format!("verify metadata: {e}"))?;
    if forbidden(&vm) { return Err(format!("forbidden namespace after verify: {}", asset.display())); }
    ...
    println!("REPACK PASS '{}' kind={} entries={} bytes:{}->{} body:{}", asset.display(), header.content_kind, header.entry_count, old.len(), rebuilt.len(), body.len());
    Ok(())
}
...
fn main() -> Result<(), String> {
    ...
    for (line_no, line) in text.lines().enumerate() {
        ...
        let (asset, body) = line.split_once('\t').ok_or_else(|| format!("manifest line {} missing tab", line_no + 1))?;

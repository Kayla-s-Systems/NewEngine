...
use newengine_assets_api::{
    decode_list_file_envelope, encode_list_file, ListFileEncodeRequest, LIST_FILE_CONTENT_KIND_YTYP,
};
...
fn main() -> Result<(), String> {
    ...
    let output = PathBuf::from(
        args.next()
            .unwrap_or_else(|| existing.clone().into_os_string()),
    );
    ...
    println!(
        ...
        output.display(),
        verify.body.len(),
        verify.header.entry_count,
        verify.metadata.entries.len(),
        verify.header.stable_file_id,
        rebuilt.len()
    );
    Ok(())
}

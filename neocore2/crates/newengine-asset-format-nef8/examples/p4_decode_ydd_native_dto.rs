...
fn main() {
    ...
    let path = args.next().map(PathBuf::from).unwrap_or_else(|| {
        eprintln!("usage: p4_decode_ydd_native_dto <artifact> <logical-path>");
        process::exit(2);
    });
    let logical_path = args.next().unwrap_or_else(|| "fixture.ydd".to_owned());
    ...
    let document = newengine_asset_format_nef8::ydd_binary::decode_ydd_binary_body(&decoded.body)
        .unwrap_or_else(|error| {
            eprintln!("YDD runtime domain decode failed: {error}");
            process::exit(1);
        });
    println!(
        "{}",
        serde_json::to_string(&document).expect("serialize YDD runtime DTO")
    );
}

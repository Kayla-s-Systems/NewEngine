...
#[inline]
fn definition_requires_graph_resolution(mode: AuthoredFpsDefinitionApplyMode) -> bool {
    matches!(mode, AuthoredFpsDefinitionApplyMode::InstantiateMarker)
}

pub(super) fn instantiate_authored_definitions(
    world: &mut newengine_ecs::World,
    root: EntityId,
    definitions: &[AuthoredFpsDefinitionInstanceSpec],
) {
    if definitions.is_empty() {
        return;
    }
    newengine_ulog_api::ulog::debug!(
        "definitions.runtime: fps-authored definition batch count={} policy='.ymap placements declare apply_mode; .ytyp dependencies are graph inputs, not implicit render/spawn commands'",
        definitions.len()
    );
    for spec in definitions {
        ...
        let graph = resolve_authored_fps_asset_graph(&spec.definition_ref).unwrap_or_else(|| {
...
mod tests {
    ...
    fn metadata_only_definitions_skip_full_graph_resolution() {
        assert!(!definition_requires_graph_resolution(
            AuthoredFpsDefinitionApplyMode::MetadataOnly
        ));
        assert!(definition_requires_graph_resolution(
            AuthoredFpsDefinitionApplyMode::InstantiateMarker
        ));
    }
}

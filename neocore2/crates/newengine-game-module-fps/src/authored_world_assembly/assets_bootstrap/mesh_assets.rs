//! Mesh asset import helpers for the FPS authored scene bootstrap.
//!
...
pub(super) fn load_ne3d_mesh_asset(logical_path: &str) -> Result<PrimitiveMesh, String> {
    ...
    if !meta.contains("kalitech.model3d.meta.v1") {
        newengine_ulog_api::ulog::warn!(
            "fps-authored: geometry asset meta is not model3d schema path='{}' meta='{}'",
            logical_path,
            meta
        );
    }
...
pub(super) fn ensure_skydome_primitive(
    ...
    if logical_path.is_empty() {
        newengine_ulog_api::ulog::info!(
            "fps-authored: skydome skipped reason='no authored .ytyp sky mesh resolved' policy='.ytyp owns SkyDome description; no procedural fallback'"
        );
        return None;
    }
    ...
    match load_skydome_mesh_asset(logical_path) {
        Ok(mesh) => {
            ...
            newengine_ulog_api::ulog::info!(
                "fps-authored: skydome imported through AssetManager path='{}' vertices={} indices={} policy='.ytyp -> .ydd/.ne3d -> engine.assets'",
                logical_path,
                vertex_count,
                index_count
            );
        ...
        Err(e) => {
            newengine_ulog_api::ulog::warn!(
                "fps-authored: skydome mesh import failed path='{}' err='{}'; authored dome disabled policy='no procedural fallback; fix .ytyp/.ydd/.ne3d asset graph'",
                logical_path,
                e
            );
            None

...
impl Transform {
    ...
    #[inline]
    pub fn from_yaw_pitch_roll(position: Vec3, yaw: f32, pitch: f32, roll: f32, scale: Vec3) -> Self {
        // Note: Euler order chosen to be stable for typical game-camera usage.
...
///
/// This component is generated by `propagate_transforms()` together with `GlobalTransform`.
/// It exists to serve typical gameplay needs (camera controllers, character controllers, etc.)
/// without forcing every system to decompose matrices manually.
#[derive(Clone, Copy, Debug, Default)]

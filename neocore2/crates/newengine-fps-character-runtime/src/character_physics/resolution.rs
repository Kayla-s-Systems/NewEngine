...
use newengine_model_contact_api::{
    ModelFootGroundSample, ModelFootGroundState, ModelFootPoseState, ModelFootSide,
    ModelGroundPlane,
};
use newengine_physics_api::PhysicsQueryHitDto;
use newengine_sim::{CharacterMotor, Velocity};

use super::queries::{foot_probe_max_t, foot_probe_owner, ground_probe_max_t, stand_probe_owner};
use super::tuning::tuning;

pub fn resolve_character_query_hits(
    world: &mut World,
    fixed_tick: u64,
    ...
    reset_foot_ground_states(world, fixed_tick);
    let mut consumed = BTreeSet::new();
    let mut blocked_stand_probes = BTreeSet::new();

    for hit in hits {
        if let Some((player, side)) = foot_probe_owner(world, hit.seq) {
            consumed.insert(hit.seq);
            apply_foot_query_hit(world, fixed_tick, player, side, *hit);
            continue;
        }
...
fn reset_foot_ground_states(world: &mut World, fixed_tick: u64) {
    let players = world
        .query::<ModelFootPoseState>()
        .map(|(player, _)| player)
        .collect::<Vec<_>>();
    for player in players {
        let next = ModelFootGroundState {
            revision: fixed_tick,
            ..Default::default()
        };
        if let Some(state) = world.get_mut::<ModelFootGroundState>(player) {
            *state = next;
        } else {
            let _ = world.insert(player, next);
        }
    }
}

fn apply_foot_query_hit(
    world: &mut World,
    fixed_tick: u64,
    player: EntityId,
    side: ModelFootSide,
    ...
    if !hit.distance.is_finite()
        || !(0.0..=foot_probe_max_t(tuning.contact_skin)).contains(&hit.distance)
    {
        return;
    }

    let point = Vec3::new(hit.position[0], hit.position[1], hit.position[2]);
    if !point.is_finite() {
        return;
    }
    ...
    if normal.y.clamp(-1.0, 1.0).acos() > tuning.max_slope_radians {
        return;
    }

    if world.get::<ModelFootGroundState>(player).is_none() {
        let _ = world.insert(
            player,
    ...
    let sample = ModelFootGroundSample {
        plane: ModelGroundPlane::new(point, normal),
        surface_key: Some(hit.entity),
    };
    if let Some(state) = world.get_mut::<ModelFootGroundState>(player) {
        state.revision = fixed_tick;
        match side {
            ModelFootSide::Left => state.left = sample,
            ModelFootSide::Right => state.right = sample,
        }
    }
}
...
fn apply_ground_query_hit(
    ...
    let vertical_velocity = world
        ...
        .unwrap_or(0.0);
    if !vertical_velocity.is_finite() || vertical_velocity > tuning.ground_probe_max_upward_velocity
    {
        return;
    }
...
mod tests {
    ...
    use newengine_math::Vec3;
    use newengine_model_contact_api::{ModelFootGroundState, ModelFootPoseState, ModelFootSide};
    ...
    #[test]
    fn per_foot_queries_preserve_independent_surface_and_plane_identity() {
        use super::super::queries::foot_probe_query_seq;
        ...
        let left_surface = world.spawn();
        let right_surface = world.spawn();
        let _ = world.insert(
            player,
            ModelFootPoseState::from_world_positions(
                1,
                Vec3::new(-0.18, 0.08, 0.0),
        ...
        let hits = [
            ...
            PhysicsQueryHitDto {
                ...
                normal: [0.0, 0.984_807_7, 0.173_648_2],
                distance: 0.325,
            },
        ];

        resolve_character_query_hits(&mut world, 77, &hits, &player_key_map(player));
        let feet = world
            .get::<ModelFootGroundState>(player)
            .copied()
            .expect("per-foot ground state");
        assert_eq!(feet.revision, 77);
        assert_eq!(feet.left.surface_key, Some(left_surface.stable_u64()));
        assert_eq!(feet.right.surface_key, Some(right_surface.stable_u64()));
        assert!(feet.left.plane.valid && feet.right.plane.valid);
        assert!((feet.left.plane.point_world.y - 0.0).abs() <= 1.0e-6);
        assert!((feet.right.plane.point_world.y - 0.025).abs() <= 1.0e-6);
        assert!(feet.right.plane.normal_world.z > 0.17);
    }
}

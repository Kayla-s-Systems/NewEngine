...
#[test]
fn abby_rifle_final_composed_prop_frame_is_terminal_weapon_handle_frame() {
    // Frame 0 after the current-rig TLOU composition:
    //   abby-mm-rifle-aim-idle -> stand-aim-add -> stand-aim-arms -> stand-aim-hands.
    // The l/r prop attachments converge to this same frame (≈0.21 mm positional error,
    // abs quaternion dot ≈0.99999999). Its local +Z already points along model forward, so native
    // rifle attachment must not apply another recovered/reference-rig orientation basis.
    let mut p = fixture();
    ...
    let socket_rotation = Quat::from_xyzw(
        -0.024_008_995,
        0.999_710_5,
        -0.000_556_152_4,
        -0.001_487_134,
    ...
    assert!(
        muzzle_forward.dot(-Vec3::Z) > 0.95,
        "final composed rifle prop frame must point the weapon forward, got {muzzle_forward:?}"
    );
    assert!(
        weapon_handle_rotation(&p, root).dot(socket_rotation).abs() > 0.999_999,
        "native prop frame is already the handle orientation and must not be re-based"
    );
    let error = weapon_handle_frame_error_from_authored_socket(&p, root, socket)
        .expect("socket frame error");
    assert!(error.position_m <= 1.0e-6);
    assert!(error.angular_degrees <= 0.001);
}
...
fn abby_rifle_bilateral_prop_pair_fuses_one_common_weapon_frame() {
    ...
    let left_position = Vec3::new(0.146_812_03, 1.322_966_5, -0.3058637);

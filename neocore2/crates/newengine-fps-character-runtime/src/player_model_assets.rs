...
> {
    ...
    if let Some(metadata) = skeleton.as_ref() {
        newengine_ulog_api::ulog::info!(
            "fps-character: player skeleton metadata bound source='{}' skeleton='{}' format='{}' bytes={} joints={} status='{}'",
            bundle.source,
            metadata.source,
    ...
    for (part_index, part) in bundle.parts.into_iter().enumerate() {
        ...
        if !prims.is_registered(primitive_id) {
            ...
            newengine_ulog_api::ulog::debug!(
                "fps-character: player model part registered source='{}' part={} slot='{}' vertices={} indices={} material='{}' policy='ydd->nemat->ytd'",
                bundle.source,
                part_index,
                part.material_slot,
                vertex_count,
            ...
            (first_person_primitive_id, first_person_build)
        {
            if !prims.is_registered(first_person_id) {
                ...
                newengine_ulog_api::ulog::info!(
                    "fps-character: first-person owner-body topology derived source='{}' part={} slot='{}' world_primitive={} fpp_primitive={} removed_triangles={} cap_triangles={} cap_loops={} fpp_indices={} policy='chest-visible neck-shell-removed neckline-sealed same-skin-stream'",
                    bundle.source,
                    part_index,
                    part.material_slot,
                    primitive_id.0,
    ...
    if registered_parts > 0 {
        newengine_ulog_api::ulog::info!(
            "fps-character: player model registered source='{}' parts={} vertices={} indices={} materials={}",
            bundle.source,
            registered_parts,
            registered_vertices,
            registered_indices,
    ...
    if let Some(dictionary) = bundle.texture_dictionary.as_deref() {
        newengine_ulog_api::ulog::info!(
            "fps-character: player model texture dictionary bound source='{}' dictionary='{}' materials={}",
            bundle.source,
            dictionary,
            out.len()
        );
    ...
    if let Some(properties_ref) = bundle.properties_ref.as_deref() {
        newengine_ulog_api::ulog::info!(
            "fps-character: player model properties descriptor bound source='{}' properties_ref='{}' policy='.ydd slots -> .ytyp material bindings -> .nemat/.ytd'",
            bundle.source,
            properties_ref
        );
    }
    ...
    if !bundle.collisions.is_empty() {
        newengine_ulog_api::ulog::info!(
            "fps-character: player model collision bindings derived source='{}' collisions={}",
            bundle.source,

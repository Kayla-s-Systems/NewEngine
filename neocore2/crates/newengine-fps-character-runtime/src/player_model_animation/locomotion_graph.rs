const FPS_CHARACTER_RUNTIME_DATA_OWNER: &str = "engine.fps.character";
...
fn compile_game_ready_locomotion_graph(
    ...
    let startup = newengine_core::startup::last_startup_config()
        .ok_or_else(|| "FPS character locomotion graph requires startup configuration".to_owned())?;
    let graph_ref = newengine_core::plugin_runtime_data_value(
        startup,
        FPS_CHARACTER_RUNTIME_DATA_OWNER,
    ...
    global_compiled_animation_graph_store().load_or_compile_with_variant(
        ...
        variant_key,
        {
            ...
            move |logical_path| {
                if logical_path.eq_ignore_ascii_case(&graph_ref) {
                    ...
                    Err(format!(
                        "unexpected FPS character locomotion graph asset path '{logical_path}'"
                    ))
                }
            }
        },
        |reference| {
            ...
            clips[slot]
                ...
                .ok_or_else(|| {
                    format!(
                        "FPS character locomotion graph alias '{reference}' resolved to empty slot={slot}"
                    )
                })
        },
    )

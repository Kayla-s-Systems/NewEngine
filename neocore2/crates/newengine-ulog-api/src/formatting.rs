//! Shared human-readable ULOG formatting helpers.
...
pub fn emit_boxed_kv<K>(title: &str, rows: &[(K, String)])
where
    K: AsRef<str>,
{
    if !crate::ulog::debug_enabled() {
        return;
    }
    ...
    crate::ulog::debug!("+{}", "-".repeat(inner_width + 2));
    crate::ulog::debug!("| {}", pad_right(title, inner_width));
    ...
    for (key, value) in rows {
        ...
        for (index, line) in wrapped.iter().enumerate() {
            if key_width == 0 {
                crate::ulog::debug!("| {}", pad_right(line, inner_width));
                continue;
            }

            if index == 0 {
                let rendered = format!("{:<key_width$} : {}", key, line, key_width = key_width);
                crate::ulog::debug!("| {}", pad_right(&rendered, inner_width));
            } else {
...
pub fn emit_prefixed_table(prefix: &str, title: &str, headers: &[&str], rows: &[Vec<String>]) {
    if !crate::ulog::debug_enabled() {
        return;
    }
...
fn emit_prefixed_line(prefix: &str, line: &str) {
    if prefix.is_empty() {
        crate::ulog::debug!("{}", line);
    } else {
        crate::ulog::debug!("{} {}", prefix, line);
    }
}

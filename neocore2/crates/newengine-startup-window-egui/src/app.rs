...
use newengine_core::startup::{StartupConfig, WindowPlacement};
...
use newengine_core::startup_window::{ShadowQuality, StartupLaunchSettings};
...
impl PreStartGraphicsApp {
    ...
    pub(super) fn render_pressure(&self) -> RenderPressure {
        ...
        score += match graphics.shadow_cascade_count {
            4 => 2,
            3 => 1,
            _ => 0,
        };
        score += match graphics.shadow_map_resolution {
            4096 => 3,
            2048 => 1,
            _ => 0,
        };
        score += if graphics.lod_distance_scale > 1.0 {
            ((graphics.lod_distance_scale - 1.0) * 4.0).ceil() as u32
        } else {
            0
        };

...
fn bind_world_item_model_from_decoded(
    ...
    newengine_ulog_api::ulog::info!(
        "fps content world item model bound id='{}' owner={:?} model='{}' parts={} center={:?} policy='ItemPickup -> YDD/NEMAT exact authored visual; fallback primitive removed after admission'",
        pickup_id,
        owner,
...
fn try_spawn_deferred_world_item(
    ...
    newengine_ulog_api::ulog::info!(
        "fps content inventory pickup spawned id='{}' item='{}' entity={:?} quantity={} auto_equip={} position={:?} rotation_ypr={:?} model={:?}",
        pending.spec.id,
        item_name,
        entity,
        pending.spec.quantity,
            ...
            Err(error) => {
                ...
                if attempts == 1 || attempts.is_multiple_of(60) {
                    newengine_ulog_api::ulog::warn!(
                        "fps content runtime world-item admission deferred owner={} attempt={} err='{}' policy='authored model never persists as fallback primitive'",
                        owner.stable_u64(),
                        attempts,
                        error,
                    );
        ...
        match try_spawn_deferred_world_item(world, prims, mats, &pending) {
            ...
            Err(error) => {
                ...
                if pending.attempts == 1 || pending.attempts % 60 == 0 {
                    newengine_ulog_api::ulog::warn!(
                        "fps content inventory pickup admission deferred id='{}' item={:?} attempt={} err='{}'",
                        pending.spec.id,
                        pending.spec.item,
                        pending.attempts,
                        error,
                ...
                if pending.attempts < 300 {
                    ...
                    newengine_ulog_api::ulog::error!(
                        "fps content inventory pickup admission abandoned id='{}' item={:?} attempts={} err='{}'",
                        pending.spec.id,
                        pending.spec.item,
                        pending.attempts,
                        error,

...
use newengine_fps_content_runtime::{embedded_test_content_provider, ensure_fps_player_loadouts};
...
fn default_medkit_item_id() -> ItemId {
    ItemId::from_name("consumable.medkit.standard").expect("valid test medkit id")
}

fn default_fps_loadout_id() -> ItemId {
...
#[test]
fn loadout_reset_and_active_weapon_drop_falls_back_without_orphan_weapon_state() {
    let mut world = World::new();
    install_fps_content(&mut world);
    ...
    let inventory = world.get::<PlayerInventory>(owner).expect("inventory");
    assert_eq!(inventory.active_slot, Some(EquipmentSlot::Sidearm));
    ...
    let fallback = world
        .get::<EquippedWeaponBinding>(owner)
        .copied()
        .expect("ranked fallback weapon binding");
    assert_eq!(fallback.slot, Some(EquipmentSlot::Sidearm));
    assert_eq!(fallback.weapon.weapon_type, WeaponType::Firearm);
    assert_eq!(fallback.weapon.rank, 200);
    assert!(world.get::<PlayerWeaponState>(owner).is_some());
}

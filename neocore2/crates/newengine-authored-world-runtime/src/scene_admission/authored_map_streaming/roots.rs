...
#[derive(Default)]
pub(super) struct AuthoredMapCellRoots {
...
#[derive(Default)]
pub(super) struct AuthoredMapPrimitiveResidency {
...
pub(super) fn record_static_world_primitive_residency(
    ...
    decoded: &[newengine_model_runtime::ydd_runtime::DecodedRuntimeYddMeshPart],
) {
    let Some(coord) = prefab.authored_cell else {
        return;
    };
    if world.resource::<AuthoredMapPrimitiveResidency>().is_none() {
        world.insert_resource(AuthoredMapPrimitiveResidency::default());
    }
    let domain = static_world_prefab_domain(prefab);
    let Some(residency) = world.resource_mut::<AuthoredMapPrimitiveResidency>() else {
        return;
    };
    let cell = residency
        .cell_primitives
...
fn ensure_cell_root(
    ...
    if world.resource::<AuthoredMapCellRoots>().is_none() {
        world.insert_resource(AuthoredMapCellRoots::default());
    }
    if let Some(registry) = world.resource_mut::<AuthoredMapCellRoots>() {
...
fn take_domain_root(
    ...
    let registry = world.resource_mut::<AuthoredMapCellRoots>()?;
...
fn remove_empty_cell_root(world: &mut newengine_ecs::World, coord: CellCoord) -> Option<EntityId> {
    ...
    world
        .resource_mut::<AuthoredMapCellRoots>()

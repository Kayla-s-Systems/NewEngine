...
struct InputSystemsEnabledState {
    raw: bool,
    bindings: bool,
    actions: bool,
    gamepad: bool,
    camera_look: bool,
    gameplay_movement: bool,
    ui_navigation: bool,
}
...
impl InputRuntimeSystems {
    ...
    pub fn observe_frame(
        ...
        self.apply_disabled_system_suppression(
            InputSystemsEnabledState {
                raw: raw_enabled,
                bindings: bindings_enabled,
                actions: actions_enabled,
                gamepad: gamepad_enabled,
                camera_look: camera_look_enabled,
                gameplay_movement: gameplay_movement_enabled,
                ui_navigation: ui_navigation_enabled,
            },
            input,
        );
    ...
    fn apply_disabled_system_suppression(
        &self,
        enabled: InputSystemsEnabledState,
        input: &mut InputActionFrameCarrier,
    ) {
        if !enabled.raw {
            input.suppress_all();
        ...
        if !enabled.actions || !enabled.bindings {
            input.suppress_actions();
        }
        if !enabled.camera_look {
            input.suppress_camera_look();
        }
        if !enabled.gameplay_movement {
            input.suppress_gameplay_movement();
        }
        if !enabled.ui_navigation {

...
use newengine_plugin_api::{
    ...
    PluginDiscoveryManifestV1, PluginDiscoveryPlatformRuntimeV1, PluginDiscoverySignatureV1,
    PluginRootV1Ref, PluginSignatureV1, LEGACY_PLUGIN_ROOT_SYMBOL_BYTES_NUL,
    PLUGIN_DESCRIPTOR_V2_SYMBOL_BYTES_NUL, PLUGIN_DISCOVERY_MANIFEST_SCHEMA_VERSION,
...
fn run() -> Result<(), String> {
    ...
    let dll = args
        .next()
        .map(PathBuf::from)
        .ok_or("usage: newengine-plugin-manifest-tool <dll> <output>")?;
    let out = args
        .next()
        .map(PathBuf::from)
        .ok_or("usage: newengine-plugin-manifest-tool <dll> <output>")?;

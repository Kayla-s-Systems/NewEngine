...
fn build_instance_slots(scene: &HairSceneV1, ranges: &[HairInstanceGpuRanges]) -> Vec<HairSlot> {
...
fn slots_to_bytes(slots: &[HairSlot]) -> Vec<u8> {
...
fn shader_set_key(shaders: &HairShaderSetV1) -> u64 {
    let mut hash = 0xcbf29ce484222325u64;
    for value in [
...
#[inline]
fn deterministic_angle(seed: u64, follower: u64) -> f32 {
    let mixed = seed
...
#[inline]
fn quality_followers(authored: u8, quality: HairQualityTier) -> usize {
...
#[inline]
fn quality_code(quality: HairQualityTier) -> f32 {
    match quality {

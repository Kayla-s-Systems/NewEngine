...
pub(in crate::scene_bridge::game_ready) fn apply_sky_visuals(
    ...
    for (entity, kind) in entities {
        match kind {
            SkyVisualKind::Dome => {
                ...
                if let Some(runtime) = world.get_mut_tracked::<SkyDomeRuntime>(entity) {
...
pub fn tick_game_ready_sky_cycle(world: &mut newengine_ecs::World, dt: f32) {
    let sky_cycle_anchor =
        crate::gameplay::scene_entity_by_role(world, crate::gameplay::SceneEntityRole::SkyCycle);
    let sun_anchor =
        crate::gameplay::scene_entity_by_role(world, crate::gameplay::SceneEntityRole::Sun);
    ...
    let spatial_shadow = spatial_cloud_shadow_from_dynamics(&frame, &dynamics);
    ...
    world.insert_resource(SkyClearColorRuntime {

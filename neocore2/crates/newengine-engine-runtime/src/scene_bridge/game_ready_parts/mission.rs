...
pub(super) fn spawn_game_ready_mission(
    ...
    for pickup in &mission.pickups {
        ...
        let _ = world.insert(
            entity,
            crate::gameplay::FpsDemoPickup {
                radius: pickup.radius,
            },
        );
        let _ = world.insert(
            entity,
            crate::gameplay::Interactable::new(format!("Collect {}", pickup.id)),
        );
    ...
    for target in &mission.targets {
        ...
        let shape = crate::gameplay::CollisionShapeDesc::Capsule {
            radius: target.scale.x.abs().max(target.scale.z.abs()).max(0.1),
            half_height: (target.scale.y.abs() - target.scale.x.abs()).max(0.1),
        };
        let _ = world.insert(
            entity,
            crate::gameplay::PhysicsBodyDesc::static_solid(shape),
        );
        let _ = world.insert(entity, crate::gameplay::Health::new(target.health));
        let _ = world.insert(entity, crate::gameplay::FpsDemoTarget);
    ...
    for goal in &mission.goals {
        ...
        let _ = world.insert(
            entity,
            crate::gameplay::FpsDemoGoal {
                radius: goal.radius,
            },
        );
        let _ = world.insert(
            entity,
            crate::gameplay::Interactable::new(format!("Extract at {}", goal.id)),
        );

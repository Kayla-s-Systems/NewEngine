...
pub(super) fn format_asset_roots(roots: &[PathBuf]) -> String {
    if roots.is_empty() {
...
fn log_surface_layer_details(logical_path: &str, profile: &GameReadyMapProfile) {
    ...
    for (index, layer) in profile.terrain.surface.layers.iter().enumerate() {
        ...
        newengine_ulog_api::ulog::info!(
            "game-ready ymap read: terrain surface layer path='{}' index={} role='{}' projected_slot='{}' texture='{}' weight={:.3} uv_scale={:.3} projected={} runtime='TerrainSurfaceLayers -> 3-channel terrain shader'",
            logical_path,
            index,
            layer.role,
            projected_slot,

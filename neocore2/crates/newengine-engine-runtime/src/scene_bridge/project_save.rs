...
pub(super) struct AuthoredMapEditJournal {
...
#[derive(Clone, Debug, Default)]
pub(crate) struct AuthoredProjectEditStatus {
    pub(crate) dirty_placements: usize,
    pub(crate) pending_creates: usize,
    pub(crate) pending_deletes: usize,
    pub(crate) last_save_succeeded: Option<bool>,
    pub(crate) last_save_message: String,
}
...
impl SceneBridge {
    ...
    pub fn save_authored_project_world(&self) -> Result<usize, String> {
        let result = self.save_authored_project_world_inner();
        let mut journal = self.authored_map_edits.lock();
        match &result {
            Ok(0) => {
    ...
    fn save_authored_project_world_inner(&self) -> Result<usize, String> {
        let edits = self.collect_authored_placement_edits()?;
        let deletes = self.authored_map_edits.lock().deletes.clone();
        let creates = edits
            .iter()
            .filter_map(|edit| {
        ...
        if !saved_entities.is_empty() {
            ...
            for entity in saved_entities {
                if scene.world().exists(entity) {
                    let _ = scene
                        .world_mut()
                        .remove::<AuthoredMapPlacementDirty>(entity);
                    let _ = scene
                        .world_mut()
                        .remove::<AuthoredMapPlacementCloneSource>(entity);
                }
            }
        }
    ...
    fn collect_authored_placement_edits(&self) -> Result<Vec<AuthoredPlacementEdit>, String> {
    ...
    pub(crate) fn authored_project_edit_status(&self) -> AuthoredProjectEditStatus {
        ...
        let (mut dirty, pending_deletes, last_save_succeeded, last_save_message) = {
            let journal = self.authored_map_edits.lock();
            (
...
mod tests {
    ...
    fn save_collection_contains_only_editor_dirty_authored_actors() {
        let bridge = SceneBridge::new(Scene::new());
        {
            ...
            let dirty = spawn_named(world, "Dirty Authored Actor");
            ...
            let clean = spawn_named(world, "Clean Authored Actor");
            ...
            let runtime_replica = spawn_named(world, "Generated Collision Replica");
            let _ = world.insert(runtime_replica, Transform::default());
            let _ = world.insert(
                runtime_replica,
                AuthoredMapPlacement::new(
                    "maps/test.ymap",
            ...
            let _ = world.insert(runtime_replica, AuthoredMapPlacementDirty);
        }

        let edits = bridge.collect_authored_placement_edits().unwrap();
...
<Prefab id="beta" position="4,5,6" rotation_ypr="0,0,0" scale="1,1,1" />
    ...
    #[test]
    fn patches_discrete_placement_with_single_quotes() {
        let mut xml =
            "<Placement id='oak-1' definition_ref='defs/oak.ytyp@oak' position='0,0,0' />"
                .to_owned();
        patch_tag_attributes_by_id(
            &mut xml,
            "Placement",
            "oak-1",
            &[("position", "7,8,9"), ("scale", "1.2,1.2,1.2")],
        )
        .unwrap();
        assert!(xml.contains("position='7,8,9'"));
        assert!(xml.contains("scale=\"1.2,1.2,1.2\""));
    }
  ...
  <Prefab id="rock" source="models/rock.ydd@rock" position="1,0,0" />
        ...
        assert!(xml.contains(
            "id=\"oak_copy_001\" source=\"models/oak.ydd@oak\" material=\"materials/oak.nemat@bark\""
        ));
    ...
    fn duplicate_uses_original_source_and_allocates_unique_authored_id() {
        ...
        let scene_lock = bridge.scene();
        let mut scene = scene_lock.write();
        let source_entity = spawn_named(scene.world_mut(), "Oak");
        ...
        let _ = scene.world_mut().insert(source_entity, authored.clone());

        let (duplicate, origin) = bridge
            .prepare_authored_duplicate(scene.world(), source_entity, &authored)
            .unwrap();

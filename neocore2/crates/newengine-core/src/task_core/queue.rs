    ...
    pub(super) fn run(mut self, shared: &TaskCoreShared) {
        ...
        let Some(job) = self.job.take() else {
            ...
            self.control.publish(
                ...
                "Task completed",
                "Task completed without a task closure.",
                Some(1.0),
            );
            release_lane();
            return;
        ...
        shared.record_cpu_time_ns(self.request.lane, elapsed_ns);
        shared.running.fetch_sub(1, Ordering::AcqRel);
        self.completion.complete();
        shared.completed.fetch_add(1, Ordering::AcqRel);
        shared.completed_by_lane[lane_index].fetch_add(1, Ordering::AcqRel);
        ...
        if result.is_err() {
            ...
            self.control.publish(
                ...
                "Task failed",
                "Worker task panicked; worker recovered and continues.",
                Some(1.0),
            );
...
impl TaskCoreShared {
    ...
    pub(super) fn submit(&self, job: QueuedTask) {
        ...
        job.control.publish(
            ...
            "Task scheduled",
            "Task was registered in the engine task queue.",
            Some(0.0),
        );
    ...
    fn has_schedulable_pending_work(&self) -> bool {
        ...
        for priority in TaskPriority::service_order() {
            if over_budget && priority != TaskPriority::Critical {
                continue;
            }

...
fn main() {
    let dir = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .expect("usage: load_target_plugin <dir> <id>");
    ...
    eprintln!(
        "env exclude={:?} target={:?}",
        std::env::var("NEWENGINE_PLUGIN_EXCLUDE_IDS").ok(),
        std::env::var("NEWENGINE_PLUGIN_TARGET").ok()
    );
    match manager.load_plugin_id_from_dir_with_origin(
        &dir,
        &id,
        host,

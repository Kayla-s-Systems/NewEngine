...
fn main() -> Result<(), String> {
    let path = env::args().nth(1).ok_or("usage: inspect_abby_clip FILE.ycd SELECTOR")?;
    ...
    println!("name={} joints={} frames={} rate={} duration={}", clip.name, clip.joint_count(), clip.frame_count(), clip.sample_rate_hz, clip.duration_seconds);
    for (i, tag) in clip.joint_tags.iter().copied().enumerate() {
        let p = clip.poses[i];
        println!("{} {} t={:?} q={:?} s={:?}", i, tag, p.translation, p.rotation, p.scale);
    }
    Ok(())
}

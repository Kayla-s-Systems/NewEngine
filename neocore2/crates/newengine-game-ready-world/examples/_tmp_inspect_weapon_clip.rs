...
fn main() -> Result<(), String> {
    let path = env::args().nth(1).ok_or("usage: _tmp_inspect_weapon_clip FILE.ycd")?;
    ...
    for clip in dictionary.clips.iter() {
        println!("name={} skeleton='{}' joints={} frames={} duration={}", clip.name, clip.skeleton_ref, clip.joint_count(), clip.frame_count(), clip.duration_seconds);
    }
    Ok(())
}

use super::*;

use newengine_model_domain_api::ModelSkinBinding;

#[path = "player_model_animation.rs"]
mod animation;
#[path = "player_model_assets.rs"]
mod assets;
#[path = "player_model_binding.rs"]
mod binding;
#[path = "player_model_validation.rs"]
mod validation;

pub(crate) use animation::{
    player_left_hand_weapon_frame, player_rifle_ready_body_frames, player_rifle_view_forward_model,
    player_right_hand_prop_frame, player_right_hand_weapon_frame,
    publish_player_first_person_camera_anchors, tick_player_skin_animation,
};
pub(crate) use binding::{
    spawn_game_ready_player_model, tick_player_model_assignments, tick_player_model_grounding,
};

#[derive(Clone, Debug)]
pub(super) struct PlayerRuntimeModelPart {
    primitive_id: PrimitiveId,
    material_id: MaterialId,
    material_slot: String,
    color: [f32; 4],
    skin: Option<ModelSkinBinding>,
}

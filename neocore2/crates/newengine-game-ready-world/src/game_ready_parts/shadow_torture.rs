...
use super::materials_terrain::{spawn_game_primitive, DemoMaterials, PrimitiveSpawnSpec};
use super::sky::SkyCycleRuntime;
use super::*;
use newengine_lighting::{LocalShadowSettings, PointLight, ShadowFilter, ShadowMethod, SpotLight};
...
fn receiver_material(mats: &MaterialRegistry, receive_shadows: bool) -> MaterialId {
    ...
    mats.upsert_named(
        if receive_shadows {
            "ShadowTorture/Receiver"
        } else {
            "ShadowTorture/NoReceive"
        },
        MaterialDescriptor {
...
pub(crate) fn bootstrap_if_requested(
    world: &mut newengine_ecs::World,
    prims: &PrimitiveRegistry,
    mats: &MaterialRegistry,
    parent: EntityId,
    ...
    if !requested() || world.resource::<ShadowTortureRuntime>().is_some() {
        return;
    }
    ...
    let Some(sun) = newengine_engine_runtime::gameplay::scene_entity_by_role(
        ...
        newengine_ulog_api::ulog::warn!(
            "shadow torture: requested but scene has no semantic Sun entity; fixture disabled"
        );
        return;
    };
    ...
    let Some(player) = newengine_engine_runtime::gameplay::scene_entity_by_role(
        ...
        newengine_ulog_api::ulog::warn!(
            "shadow torture: requested but scene has no semantic Player entity; fixture disabled"
        );
        return;
    };
    ...
    for (index, z) in [4.0_f32, 14.0, 32.0, 64.0].into_iter().enumerate() {
        spawn_fixture(
            ...
            receiver,
            builtins::ID_CUBE,
            &format!("ShadowTorture/ReceiverBand/{index}"),
    ...
    for (index, z) in [5.0_f32, 13.0, 28.0, 58.0].into_iter().enumerate() {
        spawn_fixture(
            ...
            receiver,
            builtins::ID_CUBE,
            &format!("ShadowTorture/ThinCaster/{index}"),
    ...
    world.insert_resource(ShadowTortureRuntime {
        sun,
        player,
        moving_caster,
        moving_base,
    ...
    newengine_ulog_api::ulog::info!(
        "shadow torture: enabled launch='{}' csm(cascades=4 resolution=2048 max_distance=96m filter=PCSS) local(points={} spots={} max_shadowed=4 max_resolution=1024) fixtures='receiver bands + thin casters + alpha-cutout + no-receive + dynamic caster' cycle='8s animated + 4s frozen cache window' input_mode='{}'",
        TORTURE_LAUNCH_ID,
        point_count,
        spot_count,
...
pub(crate) fn tick(world: &mut newengine_ecs::World, dt: f32) {
    let Some(runtime) = world.resource::<ShadowTortureRuntime>().copied() else {
        return;
    };
    ...
    if let Some(state) = world.resource_mut::<ShadowTortureRuntime>() {
    ...
    if phase_changed {
        newengine_ulog_api::ulog::info!(
            "shadow torture: phase={} elapsed={:.2}s expectation='{}'",
            if frozen {
                "cache-window"
            } else {
                "animated-stress"

...
pub(super) fn prepare_player_skin_sidecar(
    ...
    newengine_ulog_api::ulog::info!(
        "game-ready: authored player skin sidecar prepared source='{}' joints={} mapped_master={} local_auxiliary={} suffix='{}' local_prefix='{}' policy='exact-name projection; no proximity reskin/fallback'",
        source,
...
pub(super) fn bind_prepared_player_skin_sidecar(
    ...
    newengine_ulog_api::ulog::info!(
        "game-ready: authored player skin sidecar bound player={} source='{}' parts={} mapped_master={} local_auxiliary={} palette_owner={} policy='independent exact auxiliary palette'",
        player.stable_u64(),
        prepared.source,
        part_count,
...
pub(crate) fn tick_player_skin_sidecars(world: &mut newengine_ecs::World) {
    ...
    for player in players {
        ...
        let (owner, palette) = match update {
            ...
            Err(error) => {
                newengine_ulog_api::ulog::warn!(
                    "game-ready: authored player skin sidecar palette rejected player={} err='{}' policy='retain_last_valid_palette'",
                    player.stable_u64(),
                    error
                );
                continue;
        ...
        if !world.exists(owner) {
            newengine_ulog_api::ulog::warn!(
                "game-ready: authored player skin sidecar palette owner disappeared player={} owner={}",
                player.stable_u64(),
                owner.stable_u64()
            );
            continue;

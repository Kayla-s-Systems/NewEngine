...
pub struct ViewportBridge {
    ...
    ui_input: Mutex<ViewportCameraInput>,
...
impl ViewportBridge {
    ...
    pub fn new() -> Self {
        Self {
            ...
            ui_input: Mutex::new(ViewportCameraInput::default()),
    ...
    /// Publish one coherent camera interaction packet from UI.
    #[inline]
    pub fn publish_camera_input(&self, input: ViewportCameraInput) {
        *self.ui_input.lock() = input;
    }

    /// Read and consume the latest UI camera packet atomically.
    #[inline]
    pub fn read_camera_input(&self) -> ViewportCameraInput {
        let mut state = self.ui_input.lock();
        let out = *state;
        state.dx_px = 0.0;
        state.dy_px = 0.0;
        state.wheel_y = 0.0;

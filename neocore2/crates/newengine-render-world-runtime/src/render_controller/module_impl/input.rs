...
impl ViewportInputSnap {
    ...
    pub(super) fn apply_editor_fly_navigation(
        ...
        self.speed_scalar = if fly_speed_scale.is_finite() && fly_speed_scale > 0.0 {
            fly_speed_scale.clamp(0.05, 5.0)
        } else {
            newengine_ui_api::DEFAULT_EDITOR_FLY_SPEED_SCALE
        };
...
mod tests {
    ...
    fn editor_fly_reclaims_camera_motion_but_keeps_gameplay_gated() {
        ...
        snap.apply_editor_fly_navigation(Some(&frame), true, true, 0.20);
        ...
        assert!((snap.speed_scalar - 0.20).abs() <= f32::EPSILON);
    }

    #[test]
    fn editor_fly_speed_scale_is_sanitized_and_configurable() {
        let mut snap = ViewportInputSnap::default();
        ...
        snap.apply_editor_fly_navigation(Some(&frame), true, true, 0.35);
        assert!((snap.speed_scalar - 0.35).abs() <= f32::EPSILON);

        snap.apply_editor_fly_navigation(Some(&frame), true, true, f32::NAN);
        assert!(
            (snap.speed_scalar - newengine_ui_api::DEFAULT_EDITOR_FLY_SPEED_SCALE).abs()
                <= f32::EPSILON
        );
    }
    ...
    fn editor_fly_does_not_capture_outside_viewport() {
        ...
        frame
            .mouse_down
            .insert(newengine_input_api::mouse_button::RIGHT);
        snap.apply_editor_fly_navigation(Some(&frame), false, true, 0.20);
        assert!(!snap.fly_rmb);
        assert!(!snap.look_drag);

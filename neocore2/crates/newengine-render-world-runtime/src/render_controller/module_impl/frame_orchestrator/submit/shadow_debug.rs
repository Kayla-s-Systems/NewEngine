...
pub(super) fn shadow_torture_acceptance_trace_enabled() -> bool {
    newengine_runtime_env::var("NEWENGINE_PROJECT_LAUNCH_PRESET")
        .is_some_and(|value| value.trim().eq_ignore_ascii_case("shadow_test"))
        || matches!(
            newengine_runtime_env::var("NEWENGINE_SHADOW_TORTURE_TEST").as_deref(),
            Some("1")
                | Some("true")
                | Some("TRUE")
                | Some("yes")
...
pub(super) fn shadow_receiver_debug_mode() -> f32 {
    ...
    *MODE.get_or_init(|| {
        let raw = newengine_runtime_env::var("NEWENGINE_SHADOW_RECEIVER_DEBUG").unwrap_or_default();

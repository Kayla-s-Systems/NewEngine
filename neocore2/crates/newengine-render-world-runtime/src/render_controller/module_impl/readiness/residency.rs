...
use newengine_gameplay_world_runtime::gameplay::{
    ModelRenderComponent, PreparedRenderMesh, WorldAssemblyProgress,
};
...
fn critical_physics_collision_ready(world: &newengine_ecs::World) -> LaunchReadiness {
    let static_total = world
        .query::<newengine_gameplay_world_runtime::gameplay::StaticMeshCollider>()
        .count()
    ...
    let Some(progress) = world
        .resource::<newengine_gameplay_world_runtime::gameplay::PhysicsStaticColliderSyncProgress>()
        .copied()
    else {
...
fn critical_terrain_gpu_ready(
    ...
    let min_ready = newengine_runtime_policy::streaming_policy()

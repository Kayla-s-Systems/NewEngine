...
pub(in crate::render_controller::module_impl) struct SceneMaterialLaunchPlan {
    ...
    /// Critical textures referenced by ordinary visible-world surfaces (static world/props/terrain),
    /// excluding player skin and equipped-weapon visuals. These are promoted ahead of the existing
    /// character queue and are not eligible for launch soft-timeout fallback.
    pub(super) visible_world_paths: FxHashSet<String>,
    /// Minimal visually-correct working set that must be resident before scene handoff.
    /// This is intentionally smaller than all visible-world texture bindings so secondary
    /// normal/roughness maps may continue streaming after gameplay begins.
    pub(super) launch_required_paths: FxHashSet<String>,
    ...
    let (plan, rebuild_count) = if world_dirty || material_dirty {
        ...
        if rebuild_count <= 4 || rebuild_count.is_multiple_of(32) {
            newengine_ulog_api::ulog::debug!(
                "render launch material plan: rebuilt world_tick={} previous_tick={} material_revision={} previous_material_revision={} critical={} visible_world={} launch_required={} optional={} rebuild_count={} policy='revision-driven'",
                current_tick,
                observed_tick,
                material_revision,
                observed_material_revision,
                plan.critical_paths.len(),
                plan.visible_world_paths.len(),
...
pub(super) fn build_scene_material_launch_plan(
    ...
    let mut visible_world_paths = FxHashSet::<String>::default();
    let mut launch_required_paths = FxHashSet::<String>::default();
    let mut fallback_forbidden_paths = FxHashSet::<String>::default();

    for (entity, material_ref) in world.query::<newengine_materials::MaterialRef>() {
        ...
        let player_skin = world
            ...
            .is_some();
        let player_visual_part =
            world.get::<newengine_gameplay_world_runtime::gameplay::PlayerVisualPart>(entity);
        let equipped_weapon = player_visual_part.is_some_and(|part| {
            part.kind
        ...
        let visible_world_surface = !player_skin && player_visual_part.is_none();
        if let Some(base) = plan.base_color_texture {
            if visible_world_surface || player_skin || equipped_weapon || plan.alpha_cutoff > 0.0 {
                launch_required_paths.insert(base.to_owned());
            }
        }
        if equipped_weapon {
            // Equipped weapon is first-person/near-camera and authored as a complete PBR surface.
            for path in [plan.normal_texture, plan.roughness_texture]
                .into_iter()
                .flatten()
            {
                launch_required_paths.insert(path.to_owned());
            }
        }
        for path in [
            plan.base_color_texture,
        ...
        .flatten()
        {
            ...
            if visible_world_surface {
                visible_world_paths.insert(path.clone());
            }
    ...
    for (_entity, layers) in world.query::<TerrainMaterialLayers>() {
        for path in [
            &layers.forest_base_texture,
            &layers.sand_base_texture,
            &layers.rock_base_texture,
        ] {
            unique_paths.insert(path.clone());
            visible_world_paths.insert(path.clone());
            launch_required_paths.insert(path.clone());
        }
    }
    ...
    visible_world_paths.retain(|path| !is_launch_gate_optional_texture(path));
    launch_required_paths.retain(|path| !is_launch_gate_optional_texture(path));
    critical_paths.sort_unstable_by(|a, b| {
        let a_launch = launch_required_paths.contains(a);
        let b_launch = launch_required_paths.contains(b);
        let a_world = visible_world_paths.contains(a);
        let b_world = visible_world_paths.contains(b);
        let a_hard = fallback_forbidden_paths.contains(a);
        let b_hard = fallback_forbidden_paths.contains(b);
        b_launch
            .cmp(&a_launch)
            .then_with(|| b_world.cmp(&a_world))
            .then_with(|| b_hard.cmp(&a_hard))
            .then_with(|| a.cmp(b))
    });
...
fn extend_launch_plan_with_model_materials(
        ...
        world.query::<newengine_gameplay_world_runtime::gameplay::ModelRenderComponent>()
    {
        ...
        for part in &bundle.parts {
            ...
            if let Some(base) = material.base_color_texture {
                if visible_world_surface
                    || player_skin
                    || equipped_weapon
                    || material.alpha_cutoff > 0.0
                {
                    plan.launch_required_paths.insert(base.to_owned());
                }
            }
            if equipped_weapon {
                for path in [material.normal_texture, material.roughness_texture]
                    .into_iter()
                    .flatten()
                {
                    plan.launch_required_paths.insert(path.to_owned());
            ...
            .flatten()
            {
                if is_launch_gate_optional_texture(path) {
                    ...
                    let path = path.to_owned();
                    if visible_world_surface {
                        plan.visible_world_paths.insert(path.clone());
                    }
    ...
    plan.visible_world_paths
        .retain(|path| !is_launch_gate_optional_texture(path));
    plan.launch_required_paths
        .retain(|path| !is_launch_gate_optional_texture(path));
    plan.critical_paths.sort_unstable_by(|a, b| {
        let a_launch = plan.launch_required_paths.contains(a);
        let b_launch = plan.launch_required_paths.contains(b);
        let a_world = plan.visible_world_paths.contains(a);
        let b_world = plan.visible_world_paths.contains(b);
...
pub(super) fn critical_scene_materials_ready(
    ...
    let mut visible_world_waiting = 0_u32;
    let mut visible_world_failed = 0_u32;
    let mut launch_required_waiting = 0_u32;
    let mut launch_required_failed = 0_u32;

    for path in &plan.critical_paths {
        let fallback_forbidden = plan.fallback_forbidden_paths.contains(path);
        let visible_world = plan.visible_world_paths.contains(path);
        let launch_required = plan.launch_required_paths.contains(path);
        if launch_required && visible_world {
            this.prioritize_material_texture(path);
        } else if launch_required || fallback_forbidden {
        ...
        match this.material_texture_ready_state(r, path, "render.launch_gate") {
            ...
            MaterialTextureReadyState::Failed => {
                ...
                if visible_world {
                    visible_world_failed = visible_world_failed.saturating_add(1);
                }
                if launch_required {
                    launch_required_failed = launch_required_failed.saturating_add(1);
                }
                if this.frame.frame_index <= 4 || this.frame.frame_index.is_multiple_of(120) {
                    newengine_ulog_api::ulog::warn!(
                        "render launch texture failed path='{}' fallback_forbidden={} visible_world={} policy='failed material texture must remain diagnosable'",
                        path,
                        fallback_forbidden,
                        visible_world,
                    );
            ...
            MaterialTextureReadyState::Waiting => {
                ...
                if visible_world {
                    visible_world_waiting = visible_world_waiting.saturating_add(1);
                }
                if launch_required {
                    launch_required_waiting = launch_required_waiting.saturating_add(1);
                }
            }
        }
    }
    ...
    if launch_required_waiting > 0 || launch_required_failed > 0 {
        let launch_required_total = plan.launch_required_paths.len() as u32;
        LaunchReadiness::pending(
            format!(
                "waiting for launch-critical texture working set ready={}/{} waiting={} failed={} visible_world_waiting={} policy='base/alpha/player/weapon first-frame surfaces must be resident; secondary visible maps may continue streaming after handoff'",
                launch_required_total
                    .saturating_sub(launch_required_waiting)
                    .saturating_sub(launch_required_failed),
                launch_required_total,
                launch_required_waiting,
                launch_required_failed,
                visible_world_waiting,
            ),
            waiting.max(launch_required_waiting.saturating_add(launch_required_failed)),
            total,
            failed,
        )
    } else if fallback_forbidden_waiting > 0 {

...
impl RenderUiNodeSurfaceState {
    ...
    pub(super) fn update(
        ...
        if !newengine_runtime_policy::render_runtime_policy().primary_ui_enabled {
            self.open = false;
    ...
    fn ensure_navigation_document_loaded(&mut self, frame_index: u64) -> bool {
        ...
        let Some(document_ref) = newengine_runtime_env::var(
            newengine_ui_navigation_api::ENGINE_PRIMARY_UI_DOCUMENT_REF_ENV,
        )

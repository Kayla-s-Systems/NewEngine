...
use newengine_scene_bridge_runtime::scene_bridge::EngineViewGatewayFrame;
...
impl WorldFrameState {
    pub(super) fn require_authoritative_camera(
        ...
        if !newengine_camera_gateway_runtime::camera_gateway_route_is_authoritative_in_current_host_context() {
...
pub(super) enum PlayableFrameOutcome {
    ...
    /// Native presentation was intentionally deferred by the backend (for example bounded WSI
    /// back-pressure). No frame was opened/submitted; retry on a later host redraw.
    BackendDeferred,
    EndedEarly {
...
mod authoritative_camera_tests {
    ...
    use newengine_camera_gateway_runtime::{
        CameraGatewayBridge, CameraGatewayFrame, EngineViewFrame,
    };

fn compile_windows_app_resources() {
    #[cfg(windows)]
    {
        let manifest_dir = std::path::PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap_or_default());
        let candidates = [manifest_dir.join("assets").join("editor.ico"), manifest_dir.join("assets").join("app.ico")];
        let Some(icon) = candidates.iter().find(|p| p.is_file()) else { return; };
...
fn main() { compile_windows_app_resources(); }

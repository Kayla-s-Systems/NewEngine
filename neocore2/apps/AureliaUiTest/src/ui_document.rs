...
use newengine_ui_api::{
    UiCompiledDocument, UiNodeRequest, UiNodeRequestSourceKind, UiNodeTreeRequest, UiRuntimeNodeKind,
    UiSurfaceAdmissionPolicy, UiSurfaceAnchor, UiSurfaceStyle, UI_FONT_ASSET_BRAND,
    UI_FONT_ASSET_EDITOR_DISPLAY, UI_FONT_ASSET_EDITOR_SANS, UI_THEME_NORTHSTAR_DEFAULT,
};
...
pub fn build_aurelia_ui_test_request(
    ...
    request.diagnostics.push(format!(
        ...
        UI_FONT_ASSET_EDITOR_SANS,
        UI_FONT_ASSET_EDITOR_DISPLAY,
        UI_FONT_ASSET_BRAND,
    ));
    request
}
...
fn compile_showcase_document_to_request() -> Result<UiNodeTreeRequest, String> {
    ...
    let mut root = response
        ...
        .root
        .clone()
        .ok_or_else(|| format!("engine.assets.ui compiled '{}' without root node", response.document_ref))?;
    if root.id.trim().is_empty() {
        root.id = SURFACE_ID.to_owned();
    }
    let mut diagnostics = vec![format!("Aurelia UI Showcase authored by packed {}", SHOWCASE_DOCUMENT_REF)];
    diagnostics.extend(response.warnings.into_iter().map(|warning| format!("engine.assets.ui warning: {warning}")));
...
fn patch_runtime_values(root: &mut UiNodeRequest, frame_index: u64, click_count: u64, route_status: &AureliaUiTestRouteStatus) {

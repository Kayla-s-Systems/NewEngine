...
layout(set = 1, binding = 0) uniform sampler2D grass_albedo_alpha;
layout(push_constant) uniform GrassMaterialPush { vec4 sun_dir_intensity; vec4 sun_rgb_ambient; vec4 material_params; } pc;
...
void main() {
    vec4 albedo = texture(grass_albedo_alpha, v_uv);
    if (vegetation_alpha_cutout(albedo.a, pc.material_params.x, v_lod_fade) < 0.5) discard;
    vec3 lit = vegetation_wrap_lighting(v_normal_ws, pc.sun_dir_intensity.xyz, pc.sun_rgb_ambient.rgb * pc.sun_dir_intensity.w, pc.material_params.y);
    vec3 ambient = pc.sun_rgb_ambient.www * vec3(0.42, 0.52, 0.36);
